...
describe("clob orderbook", () => {
  ...
  it("sorts bids descending and asks ascending by price", () => {
    book.placeOrder(
      makeOrder({ side: "buy", priceLamports: bn(800), quantity: bn(10) })
    );
    book.placeOrder(
      makeOrder({ side: "buy", priceLamports: bn(1200), quantity: bn(10) })
    );
    book.placeOrder(
      makeOrder({ side: "sell", priceLamports: bn(900), quantity: bn(10) })
    );
    book.placeOrder(
      makeOrder({ side: "sell", priceLamports: bn(700), quantity: bn(10) })
    );
  ...
  it("market buy matches the cheapest ask and fills it fully", () => {
    const ask1 = makeOrder({
      side: "sell",
      priceLamports: bn(900),
      quantity: bn(40),
    });
    const ask2 = makeOrder({
      side: "sell",
      priceLamports: bn(700),
      quantity: bn(30),
    });
    ...
    expect(
      asks.find((l) => l.price.eq(bn(900)))?.totalQuantity.toString()
    ).toBe("40");
  });
  ...
  it("matches across multiple levels when one level is exhausted", () => {
    book.placeOrder(
      makeOrder({ side: "sell", priceLamports: bn(900), quantity: bn(30) })
    );
    book.placeOrder(
      makeOrder({ side: "sell", priceLamports: bn(950), quantity: bn(50) })
    );
    ...
    expect(
      asks.find((l) => l.price.eq(bn(950)))?.totalQuantity.toString()
    ).toBe("20");
  });
  ...
  it("exhausts ALL orders at the best price level before crossing to worse prices", () => {
    ...
    book.placeOrder(a2);
    book.placeOrder(
      makeOrder({ side: "sell", priceLamports: bn(900), quantity: bn(40) })
    );
    ...
    const asks = book.getAsks();
    expect(
      asks.find((l) => l.price.eq(bn(700)))?.totalQuantity.toString()
    ).toBe("10");
    expect(
      asks.find((l) => l.price.eq(bn(900)))?.totalQuantity.toString()
    ).toBe("40");
...
describe("clob getOrdersByMaker / getOpenOrders", () => {
  it("filters by maker and open status", () => {
    ...
    book.placeOrder(
      makeOrder({ maker: PublicKey.unique(), side: "buy", quantity: bn(30) })
    );
