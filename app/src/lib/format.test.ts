...
describe("formatSol", () => {
  ...
  it("returns 0.000 for undefined", () =>
    expect(formatSol(undefined)).toBe("0.000"));
});
...
describe("solToLamports", () => {
  it("converts string 1.5", () =>
    expect(solToLamports("1.5").toNumber()).toBe(1_500_000_000));
  it("converts number 2", () =>
    expect(solToLamports(2).toNumber()).toBe(2_000_000_000));
});
...
describe("calcYesPct", () => {
  ...
  it("returns 50 for zero total (fair start)", () =>
    expect(calcYesPct(0, 0)).toBe(50));
  it("returns 50 for null values (fair start)", () =>
    expect(calcYesPct(null, null)).toBe(50));
  it("returns 100 for only yes pool", () =>
    expect(calcYesPct(100, 0)).toBe(100));
...
describe("categoryName", () => {
  ...
  it("returns Other for out of range", () =>
    expect(categoryName(99)).toBe("Other"));
});
...
describe("formatBpsPct", () => {
  ...
  it("returns — for undefined", () =>
    expect(formatBpsPct(undefined)).toBe("—"));
  it("formats 5000 as 50.0%", () => expect(formatBpsPct(5000)).toBe("50.0%"));
  it("formats 0 as 0.0%", () => expect(formatBpsPct(0)).toBe("0.0%"));
  it("formats 10000 as 100.0%", () =>
    expect(formatBpsPct(10000)).toBe("100.0%"));
  it("formats fractional bps with one decimal", () =>
    expect(formatBpsPct(125)).toBe("1.3%"));
});

describe("assertBigIntSafe", () => {
  it("returns BigInt for valid string", () =>
    expect(assertBigIntSafe("42")).toBe(42n));
  it("returns BigInt for number", () => expect(assertBigIntSafe(42)).toBe(42n));
  it("returns BigInt for bigint", () =>
    expect(assertBigIntSafe(42n)).toBe(42n));
