...
> {
  ...
  if (wallet && message && signature) {
    if (!isAdminWallet(wallet, allowed)) {
      return {
        ok: false,
        response: adminResponse(403, "Wallet is not an admin"),
      };
    }
    ...
    try {
      ...
      return {
        ok: false,
        response: adminResponse(401, "Invalid wallet address"),
      };
    }
    if (!message.startsWith(ADMIN_MESSAGE_PREFIX)) {
      return {
        ok: false,
        response: adminResponse(401, "Invalid message format"),
      };
    }
    ...
    try {
      ...
      return {
        ok: false,
        response: adminResponse(401, "Invalid signature encoding"),
      };
    }
    if (!verifySignature(message, sigBytes, pubkey)) {
      return {
        ok: false,
        response: adminResponse(401, "Signature verification failed"),
      };
    }
  ...
  return {
    ok: false,
    response: adminResponse(401, "Admin authentication required"),
  };
}
