export const ERROR_MAP: Record<string, string> = {
  ...
  InvalidThreshold:
    "Threshold must be between 1 and the number of registered guardians",
  ThresholdExceedsGuardians:
    "Cannot remove guardian: required confirmations exceed remaining guardians",
  ...
  SlippageExceeded:
    "Price moved more than your slippage tolerance — try again or widen tolerance",
  ...
  ResolutionSourceMismatch:
    "Settlement source does not match market configuration",
  BatchSizeExceeded: "Batch size exceeds the maximum allowed",
  ProposalNotPending:
    "Proposal is not in pending state — it may already be approved or rejected",
  ...
  PositionHasUnclaimedRewards:
    "Claim your rewards before closing this position",
};
...
export function getFriendlyErrorMessage(err: unknown): string {
  ...
  if (
    ...
    msg.includes(
      "Transaction results in an account (0) with insufficient funds"
    )
  ) {
