...
const RUST_ERRORS_PATH = path.join(
  __dirname,
  "..",
  "..",
  "..",
...
describe("error-map", () => {
  ...
  it("contains no message with empty string", () => {
    const empty = Object.entries(ERROR_MAP).filter(
      ([, msg]) => !msg || !msg.trim()
    );
  ...
  it("getFriendlyErrorMessage resolves newly added variants", () => {
    ...
    expect(
      getFriendlyErrorMessage(new Error("PositionHasUnclaimedRewards"))
    ).toBe(ERROR_MAP.PositionHasUnclaimedRewards);
  });
