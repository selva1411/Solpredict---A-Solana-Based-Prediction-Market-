...
const marketListCache = new Map<
  string,
  { at: number; result: MarketListResult }
>();
...
async function loadMarketList(
  ...
  const markets = rows.map((r) => {
    const marketOutcomesList = (
      outcomesByMarket.get(r.marketPubkey) ?? []
 ...
 */
export async function getReclaimableMarkets(
  wallet: string,
  cooldownMs = 7 * 24 * 3600 * 1000
): Promise<ReclaimableMarket[]> {
  ...
  return rows.map((m) => {
    const isSettledOrCancelled =
      m.status === "settled" || m.status === "cancelled";
    const alreadyReclaimed =
      m.rentReclaimedAt !== null && m.rentReclaimedAt !== undefined;
    ...
    if (alreadyReclaimed) {
      ...
      const remainingHours = Math.ceil(
        (settledAtTs + cooldownMs - now) / (3600 * 1000)
      );
    ...
    return {
      ...
      reason:
        rentLamports > 0
          ? reason
          : "Rent deposit not recorded on-chain; cannot estimate",
...
export async function getTrending(limit = 6) {
  ...
  const pubkeys = (trendingRows.rows as Array<{ market_pubkey: string }>).map(
    (r) => r.market_pubkey
  );
  if (pubkeys.length === 0) {
    const fallback = await getMarketList({
      status: "open",
      sort: "popular",
      limit,
    });
