...
export async function getUserStats(wallet: string) {
  ...
  return {
    ...
    losses: s.losses ?? 0,
    winRateBps:
      s.marketsResolved && s.marketsResolved > 0 ? s.winRateBps : null,
...
export async function getUserProfile(
  ...
  const [[followerRow], [followingRow]] = await Promise.all([
    db
      .select({ count: count() })
      .from(follows)
      .where(eq(follows.followedWallet, wallet)),
    db
      .select({ count: count() })
      .from(follows)
  ...
  const winRatePct =
    marketsResolved > 0 && winRateBps !== null
      ? Number((winRateBps / 100).toFixed(2))
      : null;
  ...
  return {
    profile: {
      ...
      avatarUrl:
        user.avatarUrl ??
        `https://api.dicebear.com/7.x/identicon/svg?seed=${user.wallet}`,
      bio: user.bio ?? "",
...
const ACHIEVEMENT_DEFS = [
  {
    ...
    check: (s: AchievementUserStats) => s.marketsTraded >= 1,
    progress: (s: AchievementUserStats) => Math.min(100, s.marketsTraded * 100),
  },
  {
    key: "first_win",
    title: "First Win",
    desc: "Win your first market",
    check: (s: AchievementUserStats) => s.winningMarkets >= 1,
    progress: (s: AchievementUserStats) =>
      Math.min(100, s.winningMarkets * 100),
  },
  {
    key: "streak_3",
    title: "Hot Streak",
    desc: "Win 3 markets in a row",
    check: (s: AchievementUserStats) => s.currentStreak >= 3,
    progress: (s: AchievementUserStats) =>
      Math.min(100, (s.currentStreak / 3) * 100),
  },
  {
    key: "streak_10",
    title: "Unstoppable",
    desc: "Win 10 markets in a row",
    check: (s: AchievementUserStats) => s.currentStreak >= 10,
    progress: (s: AchievementUserStats) =>
      Math.min(100, (s.currentStreak / 10) * 100),
  },
  {
    key: "whale_100",
    title: "Whale",
    desc: "Single trade > $100",
    check: (s: AchievementUserStats) => s.largestTradeLamports >= 1e8,
    progress: (s: AchievementUserStats) =>
      Math.min(100, (s.largestTradeLamports / 1e8) * 100),
  },
  {
    key: "whale_1k",
    title: "Mega Whale",
    desc: "Single trade > $1,000",
    check: (s: AchievementUserStats) => s.largestTradeLamports >= 1e9,
    progress: (s: AchievementUserStats) =>
      Math.min(100, (s.largestTradeLamports / 1e9) * 100),
  },
  {
    key: "market_creator",
    title: "Market Creator",
    desc: "Propose an approved market",
    check: (s: AchievementUserStats) => s.proposedMarkets >= 1,
    progress: (s: AchievementUserStats) =>
      Math.min(100, s.proposedMarkets * 100),
  },
  {
    key: "oracle_whisperer",
    title: "Oracle Whisperer",
    desc: "Win 5 crypto markets",
    check: (s: AchievementUserStats) => (s.categoryWins["Crypto"] ?? 0) >= 5,
    progress: (s: AchievementUserStats) =>
      Math.min(100, ((s.categoryWins["Crypto"] ?? 0) / 5) * 100),
  },
  {
    key: "sports_savant",
    title: "Sports Savant",
    desc: "Win 5 sports markets",
    check: (s: AchievementUserStats) => (s.categoryWins["Sports"] ?? 0) >= 5,
    progress: (s: AchievementUserStats) =>
      Math.min(100, ((s.categoryWins["Sports"] ?? 0) / 5) * 100),
  },
  {
    key: "politico",
    title: "Politico",
    desc: "Win 5 politics markets",
    check: (s: AchievementUserStats) => (s.categoryWins["Politics"] ?? 0) >= 5,
    progress: (s: AchievementUserStats) =>
      Math.min(100, ((s.categoryWins["Politics"] ?? 0) / 5) * 100),
  },
  {
    key: "top_10_weekly",
    title: "Top 10",
    desc: "Top 10 in weekly leaderboard",
    check: (s: AchievementUserStats) =>
      s.weeklyRank !== null && s.weeklyRank <= 10,
    progress: () => 100,
  },
];
...
export async function getAchievements(wallet: string) {
  ...
  if (db) {
    ...
    stats.totalVolumeLamports = lamportsValues.reduce(
      (s, v) => s + Math.abs(v),
      0
    );
    ...
    if (uniqueMarkets.size > 0) {
      ...
      const mktRows = await db
        ...
        .where(
          and(
            inArray(marketsCache.marketPubkey, pubkeys),
      ...
      for (const m of mktRows) {
        ...
        const won =
          sides.includes(outcome.toUpperCase()) || sides.includes(outcome);
        if (won) {
    ...
    try {
      const [proposalRes] = await db
        ...
        .where(
          and(
            eq(marketProposals.proposer, wallet),
            eq(marketProposals.status, "approved")
          )
        );
