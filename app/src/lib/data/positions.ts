...
export async function getPositions(wallet: string) {
  ...
  const rows = await db
    ...
    .innerJoin(
      marketsCache,
      eq(marketsCache.marketPubkey, positions.marketPubkey)
    )
  ...
  return rows.map((r) => {
    ...
    const won =
      ...
      ((r.outcomeIndex === 0 && winner === "yes") ||
        (r.outcomeIndex === 1 && winner === "no"));
    if (isSettled) {
    ...
    const poolYesBps =
      poolTotal > 0 ? Math.round((yesP / poolTotal) * 10000) : 5000;
    const currentPriceBps =
      poolTotal > 0
        ? r.outcomeIndex === 0
          ? poolYesBps
          : 10000 - poolYesBps
        : outcomeMap.get(`${r.marketPubkey}:${r.outcomeIndex}`)?.[0]
            ?.lastPriceBps ?? 5000;
...
export async function getLpPositions(wallet: string): Promise<LpPosition[]> {
  ...
  const rows = await db
    ...
    .leftJoin(
      marketsCache,
      eq(marketsCache.marketPubkey, liquidityPositions.marketPubkey)
    )
