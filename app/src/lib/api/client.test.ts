...
describe("contracts", () => {
  ...
  it("rejects malformed market list", () => {
    const r = contracts.cachedMarketList.safeParse({
      ok: true,
      markets: [{ question: "missing pubkey" }],
    });
...
describe("apiFetch", () => {
  it("throws ApiError on non-ok response with error message", async () => {
    globalThis.fetch = async () =>
      new Response(JSON.stringify({ error: "Database not available" }), {
        status: 503,
      });
    await expect(
      apiFetch("cachedMarketList", "/api/markets/cached")
  ...
  it("throws ApiError on contract mismatch", async () => {
    globalThis.fetch = async () =>
      new Response(JSON.stringify({ ok: true, markets: "not-an-array" }), {
        status: 200,
      });
    await expect(
      apiFetch("cachedMarketList", "/api/markets/cached")
