...
export function totalVolumeLocked(markets: MarketAccount[]): number {
  return markets.reduce((sum, m) => {
    return (
      sum +
      (m.account.yesPoolLamports + m.account.noPoolLamports) / 1_000_000_000
    );
  }, 0);
}
...
export function marketsEndingSoon(
  markets: MarketAccount[],
  now = Date.now() / 1000
): MarketAccount[] {
  return markets.filter((m) => {
    return (
      m.account.status === 0 &&
      m.account.endTs > now &&
      m.account.endTs - now < 86400
    );
  });
}

/** Most lopsided market (highest YES or NO %) */
export function mostLopsidedMarket(
  markets: MarketAccount[]
): MarketAccount | null {
  return markets.reduce((best, m) => {
    ...
    if (!best) return m;
    const bestTotal =
      best.account.yesPoolLamports + best.account.noPoolLamports;
...
export function poolDepthWarning(
...
export function contrarianMultiplier(
...
/** Time pressure label */
export function timePressureLabel(
  endTs: number,
  now = Date.now() / 1000
): string | null {
