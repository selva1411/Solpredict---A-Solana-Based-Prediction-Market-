...
export function normalizeLpAllocation(
 ...
 */
export function lpSplitLamports(
  option: LpAllocation,
  amountSol: number
): { yesLamports: number; noLamports: number } {
  const { yesSol, noSol } = lpSplitFor(option, amountSol);
  return {
    yesLamports: Math.round(yesSol * 1e9),
    noLamports: Math.round(noSol * 1e9),
  };
}
 ...
 */
export function lpTokensMintedFor(
  option: LpAllocation,
  amountSol: number
): number {
...
export function lpPreview(
  option: LpAllocation,
  amountSol: number,
  currentYesPoolSol: number,
  currentNoPoolSol: number
): LpPreview {
