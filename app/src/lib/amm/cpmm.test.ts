...
describe("cpmm (probability-based constant-product AMM) parity with amm_math.rs", () => {
  it("balanced pools price each side at 50% and sum to 1", () => {
    ...
    expect(getSpotPriceYes(yes, no, 0) + getSpotPriceNo(yes, no, 0)).toBe(
      CPMM_SCALE
    );
  });
  ...
  it("sellRefundLamports < buyCostLamports for a position", () => {
    ...
    const refund = sellRefundLamports(
      { poolYes: newYes, poolNo, feeBps: fee },
      "YES",
      dy
    );
    expect(refund < cost).toBe(true);
  ...
  it("sellRefundLamports with empty pool yields 0 for both sides", () => {
    expect(
      sellRefundLamports(
        { poolYes: 0n, poolNo: 5_000_000n, feeBps: 30 },
        "YES",
        100_000n
      )
    ).toBe(0n);
    expect(
      sellRefundLamports(
        { poolYes: 5_000_000n, poolNo: 0n, feeBps: 30 },
        "NO",
        100_000n
      )
    ).toBe(0n);
