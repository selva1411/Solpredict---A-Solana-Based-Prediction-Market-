...
export async function recordTradeInDb(data: {
  ...
  try {
    ...
    const price =
      data.pricePerToken !== undefined && data.pricePerToken > 0
        ? data.pricePerToken.toString()
        : data.lamportsIn !== 0 && data.tokensOut !== 0
        ? (
            Math.abs(data.lamportsIn) /
            1e9 /
            Math.abs(data.tokensOut)
          ).toString()
        : null;
    if (price === null) {
      logger.warn(
        "recordTradeInDb: cannot derive a real price; refusing to write a fabricated trade",
        data
      );
      return null;
    }
...
export async function getRecentTradesFromDb(limit = 20) {
  ...
  try {
    const rows = await db
      ...
      .leftJoin(
        marketsCache,
        eq(trades.marketPubkey, marketsCache.marketPubkey)
      )
