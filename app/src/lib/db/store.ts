...
export async function getMarketComments(
  ...
  try {
    ...
    return rows.map((r) => ({
      ...
      authorAvatar:
        r.authorAvatar ||
        `https://api.dicebear.com/7.x/identicon/svg?seed=${r.authorWallet}`,
      content: r.content,
...
export async function recordLeaderboardSnapshot() {
  ...
  try {
    ...
    const topUsers = await db
      .select()
      .from(userStats)
      .orderBy(desc(userStats.totalVolume))
      .limit(50);
  ...
  try {
    ...
    if (rows.length > 0) {
      return rows.map((u, i) => {
        ...
        const settled = u.marketsResolved ?? wins + losses;
        const pasScore =
          settled > 0 ? Math.round((wins / settled) * 100) : null;
        return {
          rank: i + 1,
          wallet: u.wallet,
          username:
          ...
          avatarUrl:
            u.avatarUrl ||
            `https://api.dicebear.com/7.x/identicon/svg?seed=${u.wallet}`,
    ...
    const tradeAggregation = await db
      .select({
      ...
      .orderBy(drizzleSql`SUM(ABS(${trades.lamportsIn})) DESC`)
      .limit(20);
