...
function withConnectRetry<T extends (...args: never[]) => unknown>(sql: T): T {
  const retry = async <A extends unknown[]>(
    run: () => Promise<Awaited<unknown>>
  ): Promise<unknown> => {
    let attempt = 0;
    while (true) {
      try {
        ...
        if (!isTransientConnectError(err) || attempt >= CONNECT_RETRIES)
          throw err;
        await new Promise((r) =>
          setTimeout(r, CONNECT_RETRY_DELAY_MS[attempt] ?? 700)
        );
        attempt++;
      }
    }
  ...
  return new Proxy(sql, {
    ...
    get(target, prop, receiver) {
      ...
      if (prop === "query" && typeof value === "function") {
        ...
        return (...args: unknown[]) =>
          retry(() => value.apply(target, args) as Promise<unknown>);
      }
...
function createDb(): NeonHttpDatabase<typeof schema> {
  ...
  try {
    ...
    const g = globalThis as unknown as {
      __neonKeepAlive?: ReturnType<typeof setInterval>;
    };
