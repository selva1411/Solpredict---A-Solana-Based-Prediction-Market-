...
export async function getAllMarkets(options?: {
  ...
  try {
    ...
    const tradeStatsMap = new Map<
      string,
      { volume: number; volume24h: number; traders: number }
    >();
    try {
...
export async function getMarketByPubkey(
  pubkey: string
...
export async function createMarketInDb(data: {
  ...
  const pubkey =
    data.marketPubkey ||
    getMarketPda(new anchor.BN(nextId), ENV.programId).toBase58();
...
export async function updateMarketInDb(
  marketPubkey: string,
  data: Partial<{
    question: string;
    description: string;
...
export async function settleMarketInDb(
  marketIdOrPubkey: string,
  outcome: "YES" | "NO" | "yes" | "no" | "cancel"
) {
  if (!db) return false;
  ...
  await db
    ...
    .where(
      or(
        eq(marketsCache.marketPubkey, marketIdOrPubkey),
        eq(sql`CAST(${marketsCache.marketId} AS TEXT)`, marketIdOrPubkey)
      )
    );
...
export async function getMarketPriceHistory(
  marketPubkey: string,
  limit = 120
): Promise<Array<{ timestamp: Date; yesPct: number }>> {
  if (!db) return [];
  try {
    const rows = await db
      .select({
