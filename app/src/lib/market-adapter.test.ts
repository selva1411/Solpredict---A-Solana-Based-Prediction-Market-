...
describe("categoryFromIndex", () => {
  it("returns Crypto for index 0", () =>
    expect(categoryFromIndex(0)).toBe("Crypto"));
  it("returns Sports for index 1", () =>
    expect(categoryFromIndex(1)).toBe("Sports"));
  it("returns Politics for index 2", () =>
    expect(categoryFromIndex(2)).toBe("Politics"));
  it("returns Tech for index 3", () =>
    expect(categoryFromIndex(3)).toBe("Tech"));
  it("returns Other for index 4", () =>
    expect(categoryFromIndex(4)).toBe("Other"));
  it("returns Other for out-of-range index", () =>
    expect(categoryFromIndex(99)).toBe("Other"));
  it("returns Other for negative index", () =>
    expect(categoryFromIndex(-1)).toBe("Other"));
});
...
describe("lamportsToSol", () => {
  ...
  it("converts 500_000_000 to 0.5", () =>
...
describe("onChainToUiMarket", () => {
  ...
  it("returns correct id", () =>
    expect(onChainToUiMarket(makeMock()).id).toBe("test123"));
  it("returns correct question", () =>
    expect(onChainToUiMarket(makeMock()).question).toBe("Test?"));
  it("returns correct description", () =>
    expect(onChainToUiMarket(makeMock()).description).toBe("A test market"));
  it("returns Crypto category", () =>
    expect(onChainToUiMarket(makeMock()).category).toBe("Crypto"));
  it("computes yesPrice = 0.5 for equal pools", () =>
    expect(onChainToUiMarket(makeMock()).yesPrice).toBeCloseTo(0.5));
  ...
  it("returns 0 default volume24h when no enrichment", () =>
    expect(onChainToUiMarket(makeMock()).volume24h).toBe(0));
  it("returns 0 default traders when no enrichment", () =>
    expect(onChainToUiMarket(makeMock()).traders).toBe(0));
  it("returns false hot by default", () =>
    expect(onChainToUiMarket(makeMock()).hot).toBe(false));
  it("returns false trending by default", () =>
    expect(onChainToUiMarket(makeMock()).trending).toBe(false));
