...
export function onChainToUiMarket(
  ...
  const bps =
    poolTotal > 0n
      ...
      : enrichment?.lastPriceBps ??
        probabilityYesBps(DEFAULT_B, yesSupply, noSupply);
  const yesPrice = bps / 10000;
  const liquidity = lamportsToSol(
  ...
  return {
    ...
    createdAt: enrichment?.createdAt ?? m._dbCreatedAt,
    ...
    oracleFeedId: Array.isArray(m.account.oracleFeedId)
      ? "0x" + Buffer.from(m.account.oracleFeedId).toString("hex")
      : "",
...
export function cacheToUiMarket(
  ...
  const yesPool =
    typeof c.yesPoolSol === "number" && c.yesPoolSol > 0 ? c.yesPoolSol : 0;
  const noPool =
    typeof c.noPoolSol === "number" && c.noPoolSol > 0 ? c.noPoolSol : 0;
  ...
  const bps =
    poolTotal > 0
      ? Math.round((yesPool / poolTotal) * 10000)
      : c.lastPriceBps ?? 5000;
  const yesPrice = bps / 10000;
