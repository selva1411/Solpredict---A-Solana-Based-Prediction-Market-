...
export async function requireUser(
  ...
  { ok: true; identity: UserIdentity } | { ok: false; response: NextResponse }
> {
  ...
  if (wallet && message && signature) {
    if (expectedWallet && wallet !== expectedWallet) {
      return {
        ok: false,
        response: userResponse(403, "Wallet does not match request target"),
      };
    }
    ...
    try {
      ...
      return {
        ok: false,
        response: userResponse(401, "Invalid wallet address"),
      };
    }
    ...
    try {
      ...
      return {
        ok: false,
        response: userResponse(401, "Invalid signature encoding"),
      };
    }
    if (!verifySignature(message, sigBytes, pubkey)) {
      return {
        ok: false,
        response: userResponse(401, "Signature verification failed"),
      };
    }
  ...
  if (token) {
    ...
    if (session) {
      ...
      return {
        ok: true,
        identity: { wallet: session.wallet, method: "session" },
      };
    }
  }
  ...
  return {
    ok: false,
    response: userResponse(401, "User authentication required"),
  };
}
