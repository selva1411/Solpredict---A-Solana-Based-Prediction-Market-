 ...
 */
async function applyEventSafe(
  ev: Parameters<typeof import("./reducer").applyEvent>[0]
): Promise<void> {
  try {
    await applyEvent(ev);
  } catch {
  ...
  if (
    ...
    if (str.length > 15) {
      ...
      if (parsed > BigInt(Number.MAX_SAFE_INTEGER)) {
        throw new Error(
          `bnToNumber: u64 value ${str} exceeds Number.MAX_SAFE_INTEGER`
        );
      }
    }
    num = bn.toNumber();
...
export async function reconcileMarkets({
  ...
  program,
  limit = 500,
}: ReconcileOptions): Promise<number> {
  if (!getDb()) return 0;
  ...
  try {
    const configAcct = (await program.account.config.fetchNullable(
      configPda
    )) as ConfigAccount | null;
    if (configAcct) {
      nextMarketId = bnToNumber(
        (configAcct as Record<string, unknown>).marketCount
      );
    }
  } catch (e) {
    logger.warn(
      "reconcileMarkets: could not read config, defaulting nextMarketId=0:",
      e
    );
  }
  ...
  for (let marketId = 0; marketId < nextMarketId; marketId++) {
    ...
    const pda = getMarketPda(
      new (await import("@coral-xyz/anchor")).BN(marketId),
      program.programId
    );
    try {
      const acct = (await program.account.market.fetchNullable(
        pda
...
export async function reconcileTrades({
  ...
  before,
  limit = 20,
}: ReconcileOptions & { before?: string }): Promise<{
  trades: number;
    ...
    try {
      ...
      for (const ix of msg.instructions) {
        ...
        await applyEventSafe({
          ...
          pricePerToken:
            tokensOut > 0
              ? Math.abs(lamportsIn) / LAMPORTS_PER_SOL / Math.abs(tokensOut)
              : 0,
...
export async function reconcilePositions({
  program,
}: ReconcileOptions): Promise<ReconcilePositionsResult> {
  ...
  try {
    ...
    for (const a of accts) {
      ...
      const totalSpent =
        bnToNumber(account.totalSpentLamports) ??
        bnToNumber(account.totalSpent);
      ...
      if (yesAmount > 0)
        sides.push({ side: "YES", amt: yesAmount, cost: split(yesAmount) });
      if (noAmount > 0)
        sides.push({ side: "NO", amt: noAmount, cost: split(noAmount) });
  ...
  logger.info(
    `[indexer] reconcilePositions: ${positions} positions, ${trades} trades`
  );
