...
export function effectiveTradePrice(
  ...
  if (ev.pricePerToken !== undefined && ev.pricePerToken > 0)
    return ev.pricePerToken;
  ...
  try {
    ...
    await db
      ...
      .values({
        ...
        endTs: ev.endTs
          ? new Date(ev.endTs * 1000)
          : new Date(Date.now() + 3600000),
        resolveTs: ev.resolveTs
          ? new Date(ev.resolveTs * 1000)
    ...
    if (
      ...
      yesPriceBps =
        pTotal > 0
          ? Math.round((Number(ev.yesPoolLamports) / pTotal) * 10000)
          : 5000;
    } else {
...
export async function applyTradeEvent(ev: TradeEvent): Promise<void> {
  ...
  try {
    ...
    if (typeof ev.yesPoolLamports === "number")
      marketSet.yesPoolLamports = ev.yesPoolLamports;
    if (typeof ev.noPoolLamports === "number")
      marketSet.noPoolLamports = ev.noPoolLamports;
    ...
    if (
      ...
      const yesPriceBps =
        pTotal > 0 ? Math.round((pYes / pTotal) * 10000) : 5000;
...
export async function applySettleEvent(ev: SettleEvent): Promise<void> {
  ...
  try {
    ...
    const outcome =
      ev.winningOutcome === "cancel" ? undefined : ev.winningOutcome;
...
export async function applyLiquidityEvent(ev: LiquidityEvent): Promise<void> {
  ...
  try {
    ...
    if (typeof yesLp !== "number" || typeof noLp !== "number") {
      const row = await db
        .select({
          yes: marketsCache.yesPoolLamports,
          no: marketsCache.noPoolLamports,
        })
        .from(marketsCache)
        .where(eq(marketsCache.marketPubkey, ev.marketPubkey))
        .limit(1);
      yesLp =
        typeof ev.yesPoolLamports === "number"
          ? ev.yesPoolLamports
          : Number(row[0]?.yes ?? 0) + ev.yesLamports;
      noLp =
        typeof ev.noPoolLamports === "number"
          ? ev.noPoolLamports
          : Number(row[0]?.no ?? 0) + ev.noLamports;
    }
    await db
      .update(marketsCache)
      .set({
    ...
    const yesPriceBps =
      pTotal > 0 ? Math.round((Number(yesLp) / pTotal) * 10000) : 5000;
    await db
      .insert(marketOutcomes)
      .values({
        marketPubkey: ev.marketPubkey,
...
export async function applyRewardClaimedEvent(
...
export async function applyFeesWithdrawnEvent(
  ev: FeesWithdrawnEvent
): Promise<void> {
  const db = getDb();
  if (!db) return;
  try {
...
export async function applyEmergencyPauseEvent(
  ev: EmergencyPauseEvent
): Promise<void> {
  const db = getDb();
  if (!db) return;
  try {
...
export async function applyEmergencyWithdrawEvent(
  ev: EmergencyWithdrawEvent
): Promise<void> {
  const db = getDb();
  if (!db) return;
  try {
...
export async function applyRentReclaimedEvent(
  ev: RentReclaimedEvent
): Promise<void> {
  const db = getDb();
  if (!db) return;
  try {
