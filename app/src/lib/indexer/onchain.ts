...
function getProgram(): Program<Solpredict> {
  if (!programInstance) {
    ...
    const provider = new AnchorProvider(getConnection(), dummyWallet as never, {
    ...
    programInstance = new Program(
      idl,
      provider
    ) as unknown as Program<Solpredict>;
  }
...
export async function verifyTradeSignature(
  ...
  if (tx.meta?.err) {
    throw new Error(
      `Trade transaction ${signature.slice(
        0,
        8
      )}... failed on-chain and will not be recorded.`
    );
  }
  ...
  if (!msg || !meta) {
    throw new Error(
      `Trade transaction ${signature.slice(0, 8)}... is missing account data.`
    );
  }
  ...
  for (const ix of msg.instructions) {
    ...
    const tIdx = accountKeys.findIndex((k) =>
      (k.pubkey as PublicKey).equals(treasury)
    );
    const rawCost =
      tIdx >= 0 ? (postBalances[tIdx] ?? 0) - (preBalances[tIdx] ?? 0) : 0;
    ...
    try {
      ...
      yesPoolLamports = bnToNumber(
        (acct as unknown as Record<string, unknown>).yesPoolLamports
      );
      noPoolLamports = bnToNumber(
        (acct as unknown as Record<string, unknown>).noPoolLamports
      );
      yesSupply = bnToNumber(
        (acct as unknown as Record<string, unknown>).yesSupply
      );
      noSupply = bnToNumber(
        (acct as unknown as Record<string, unknown>).noSupply
      );
    } catch {
    ...
    return {
      ...
      tokensOut,
      pricePerToken:
        tokensOut !== 0 ? Math.abs(lamportsIn) / 1e9 / Math.abs(tokensOut) : 0,
...
export async function verifyLiquiditySignature(
  ...
  if (!tx) {
    throw new Error(
      `Liquidity signature ${signature.slice(
        0,
        8
  ...
  if (tx.meta?.err) {
    throw new Error(
      `Liquidity transaction ${signature.slice(
        0,
        8
      )}... failed on-chain and will not be recorded.`
    );
  ...
  for (const ix of msg.instructions) {
    ...
    if (!provider.equals(expectedProvider)) {
      throw new Error(
        "Liquidity provider does not match the transaction signer."
      );
    }
    if (!market.equals(expectedMarket)) {
    ...
    if (
      !Number.isFinite(yesLamports) ||
      !Number.isFinite(noLamports) ||
      yesLamports + noLamports <= 0
    ) {
...
export async function verifyRewardClaimSignature(
  ...
  if (tx.meta?.err) {
    throw new Error(
      `Claim transaction ${signature.slice(
        0,
        8
      )}... failed on-chain and will not be recorded.`
    );
  ...
  if (!msg || !tx.meta) {
    throw new Error(
      `Claim transaction ${signature.slice(0, 8)}... is missing account data.`
    );
  }
  ...
  for (const ix of msg.instructions) {
    ...
    const treasury = accounts[2] as PublicKey | undefined;
    if (treasury) {
      const tIdx = msg.accountKeys.findIndex((k) =>
        (k.pubkey as PublicKey).equals(treasury)
      );
      if (tIdx >= 0) {
        const delta =
      ...
      try {
        ...
        for (const event of parser.parseLogs(logs)) {
          if (event.name === "RewardsClaimed") {
            ...
            const payout = (
              data.payout as { toNumber?: () => number }
            )?.toNumber?.();
...
export async function verifyProposalSignature(
  ...
  if (!tx) {
    throw new Error(
      `Proposal signature ${signature.slice(
        0,
        8
      )}... was not found on-chain. ` +
  ...
  if (tx.meta?.err) {
    throw new Error(
      `Proposal transaction ${signature.slice(
        0,
        8
      )}... failed on-chain and will not be recorded.`
    );
...
export async function verifyRentReclaimSignature(
  ...
  if (tx.meta?.err) {
    throw new Error(
      `Reclaim transaction ${signature.slice(
        0,
        8
      )}... failed on-chain and will not be recorded.`
    );
  ...
  if (!msg || !tx.meta) {
    throw new Error(
      `Reclaim transaction ${signature.slice(0, 8)}... is missing account data.`
    );
  }

  const marketKey = new PublicKey(marketPubkey);
  const idx = msg.accountKeys.findIndex((k) =>
    (k.pubkey as PublicKey).equals(marketKey)
  );
  if (idx < 0) {
  ...
  if (preBalance === postBalance) {
    throw new Error(
      "The transaction did not change the market account's balance."
    );
  }
