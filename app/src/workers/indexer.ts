...
function parseArgs(argv: string[]): Args {
  ...
  for (const a of argv) {
    ...
    if (m) args.intervalMs = Number(m[1]);
    const n = a.match(/^--markets=(\d+)$/);
    if (n) args.marketsLimit = Number(n[1]);
    const t = a.match(/^--trades=(\d+)$/);
    if (t) args.tradesPerPass = Number(t[1]);
  }
  return args;
}
...
async function runPass(args: Args): Promise<void> {
  ...
  const markets = await reconcileMarkets({
    connection,
    program,
    limit: args.marketsLimit,
  });
  ...
  logger.info(
    `[indexer] pass done: ${markets} markets, ${trades} trades in ${
      Date.now() - started
    }ms`
  );
}
