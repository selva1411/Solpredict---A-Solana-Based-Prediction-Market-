...
export const POST = apiHandler(async (req: NextRequest) => {
  ...
  const swingingFactors = [
    `Overall ${String(
      category || "Market"
    )} momentum & macro economic sentiment`,
    `Whale liquidity positioning (${Number(yesPool).toFixed(
      2
    )} SOL YES vs ${Number(noPool).toFixed(2)} SOL NO)`,
    `Oracle price feed volatility leading up to resolution cutoff`,
  ];
  if (momentum && momentum.trades24h > 0) {
    swingingFactors.unshift(
      `On-chain flow: ${momentum.yesVolume24h.toFixed(
        2
      )} SOL into YES vs ${momentum.noVolume24h.toFixed(2)} SOL into NO (${
        momentum.trades24h
      } trades) — ${momentum.direction}`
    );
  }

  const verdict =
    Number(yesProb) > 55
      ? `Bulls hold a ${yesProb}% market lead backed by ${Number(
          yesPool
        ).toFixed(2)} SOL in YES liquidity${
          momentum && momentum.trades24h > 0
            ? `, reinforced by ${momentum.direction}.`
            : "."
        }`
      : Number(noProb) > 55
      ? `Bears dominate with ${noProb}% implied probability backed by ${Number(
          noPool
        ).toFixed(2)} SOL in NO liquidity${
          momentum && momentum.trades24h > 0
            ? `, reinforced by ${momentum.direction}.`
            : "."
        }`
  ...
  const summary = {
    ...
    historicalBaseRate:
      category === "Crypto"
        ? "72% historical settlement accuracy for oracle price thresholds."
        : "64% historical resolution rate within 48 hours.",
    recommendation:
      Number(yesProb) > 65
        ? "Bullish trend strongly favored by liquidity pool depth."
        : "Consider limit orders near spread extremes to maximize potential ROI.",
