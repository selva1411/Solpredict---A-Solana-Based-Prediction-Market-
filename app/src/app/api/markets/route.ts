...
export const POST = apiHandler(async (req: NextRequest) => {
  ...
  try {
    ...
    if (!body)
      return ok({ ok: false, error: "Invalid JSON body" }, { status: 400 });
    const {
      question,
      description,
      category,
    ...
    if (!question || !endTs) {
      return ok(
        { ok: false, error: "question and endTs are required" },
        { status: 400 }
      );
    }
