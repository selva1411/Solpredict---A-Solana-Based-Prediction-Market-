...
export const POST = apiHandler(async (req: NextRequest) => {
  ...
  if (!verifySignature(message, sigBytes, pubkey)) {
    return badRequest(
      "Signature verification failed — sign the challenge with the proposing wallet"
    );
  }
  ...
  if (
    typeof body?.oracleFeedId === "string" &&
    body.oracleFeedId.trim() !== ""
  ) {
  ...
  if (!parsed.success) {
    ...
    return badRequest(
      `Validation failed: ${issue.path.join(".")} — ${issue.message}`
    );
  }
  ...
  try {
    ...
    if (verified.proposer !== wallet) {
      return badRequest(
        "The on-chain proposal was not submitted by this wallet"
      );
    }
    ...
    const [proposal] = await db
      ...
      .values({
        ...
        category:
          ["Crypto", "Sports", "Politics", "Tech", "Other"][
            verified.category
          ] ?? "Crypto",
