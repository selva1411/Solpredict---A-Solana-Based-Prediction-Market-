  ...
  async (
    ...
    try {
      ...
      if (market.status !== "settled") {
        return badRequest(
          `Only settled markets can be disputed. Current status: ${market.status}`
        );
      }

      // Dispute window check: 24h (86400s) after settledAt
      const settledAtTs = market.settledAt
        ? new Date(market.settledAt).getTime()
        : Date.now();
      const disputeWindowMs = 86400 * 1000;
      if (Date.now() > settledAtTs + disputeWindowMs) {
        return badRequest(
          "Dispute window has expired (must dispute within 24 hours of settlement)"
        );
      }
      ...
      return ok(
        {
          ...
          dispute: row,
          message:
            "Dispute submitted successfully. Market reward claims are now frozen pending review.",
        },
        { status: 201 }
      );
    } catch (err) {
