...
export const GET = apiHandler(async (req: NextRequest, context) => {
  ...
  try {
    ...
    const walletParam =
      req.nextUrl.searchParams.get("wallet")?.trim() || undefined;
    const [poolStats, userLp] = db
      ? await Promise.all([
          db
            .select({
            ...
            .from(lpPoolStats)
            .where(eq(lpPoolStats.marketPubkey, marketPubkey))
            .limit(1),
          walletParam
            ? db
                .select({
                ...
                .where(
                  sql`${liquidityPositions.wallet} = ${walletParam} AND ${liquidityPositions.marketPubkey} = ${marketPubkey}`
                )
                .limit(1)
  ...
  try {
    ...
    if (existing.length > 0) {
      await db
        ...
        .set({
          ...
          deposited: sql`${
            liquidityPositions.deposited
          } + ${depositedSol.toFixed(9)}`,
          updatedAt: new Date(),
        })
    ...
    await db
      ...
      .onConflictDoUpdate({
        ...
        set: {
          totalLiquiditySol: sql`${
            lpPoolStats.totalLiquiditySol
          } + ${depositedSol.toFixed(9)}`,
    ...
    if (
      typeof verified.yesPoolLamports === "number" &&
      typeof verified.noPoolLamports === "number"
    ) {
      await db
        .update(marketsCache)
        .set({
