  ...
  async (
    ...
    try {
      ...
      if (market.status !== "settled" && market.status !== "cancelled") {
        return badRequest(
          `Market must be settled or cancelled. Current status: ${market.status}`
        );
      }

      if (market.rentReclaimedAt) {
        return badRequest(
          "Rent deposit has already been reclaimed for this market"
        );
      }
      ...
      const verified = await verifyRentReclaimSignature(
        signature,
        marketPubkey
      );
      ...
      return ok({
        ...
        verified: true,
        message:
          "Rent deposit reclaimed successfully (market account closed on-chain)",
      });
    } catch (err) {
