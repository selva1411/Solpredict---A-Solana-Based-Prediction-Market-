...
export const GET = apiHandler(async (req: NextRequest) => {
  ...
  const direction =
    (url.searchParams.get("direction") as "in" | "out" | null) ?? undefined;
  ...
  try {
    ...
    try {
      ...
      onChainBalanceLamports = await conn.getBalance(
        new PublicKey(overview.treasuryWallet)
      );
    } catch (e) {
    ...
    const { totalInLamports, totalOutLamports, netLedgerSol } =
      overview.ledgerTotals;
...
export const POST = apiHandler(async (req: NextRequest) => {
  ...
  const { amountSol, signature, recipient, kind, note, marketPubkey } =
    body as {
      amountSol?: number;
      signature?: string;
      recipient?: string;
  ...
  if (!amountSol || amountSol <= 0)
    return badRequest("Valid amountSol required");
