...
export const GET = apiHandler(async (req: NextRequest) => {
  ...
  try {
    ...
    const claimableTotalLamports = claimableRows.reduce(
    ...
    const breakdown = {
      tradingSol:
        rewardRows
          .filter((r) => r.kind === "trading")
          .reduce((s, r) => s + (r.amount ?? 0), 0) / 1e9,
      lpSol:
        rewardRows
          .filter((r) => r.kind === "lp")
          .reduce((s, r) => s + (r.amount ?? 0), 0) / 1e9,
    ...
    const categoriesTraded = Number(
      (categoryCountRes.rows?.[0] as Record<string, unknown> | undefined)
        ?.count ?? 0
    );
...
export const POST = apiHandler(async (req: NextRequest) => {
  ...
  if (!message || !proofSignature) {
    return badRequest(
      "x-message and x-signature headers required to prove wallet ownership"
    );
  }
  ...
  if (!verifySignature(message, sigBytes, pubkey)) {
    return badRequest(
      "Signature verification failed — you must sign the challenge with the wallet you are claiming for"
    );
  }
  ...
  if (!claimSignature) {
    return badRequest(
      "signature (the confirmed on-chain claim transaction) is required"
    );
  }
  ...
  try {
    ...
    if (!verified.claimer || verified.claimer !== wallet) {
      return badRequest(
        "The on-chain claim transaction was not made by this wallet"
      );
    }
    ...
    const totalClaimedLamports = claimableRows.reduce(
      (sum, r) => sum + (r.amount ?? 0),
      0
    );
