...
import { lamportsToSol, bnToNum, formatTs } from "@/lib/format";
import { clampQuantity } from "@/lib/utils";
import {
  buyCostLamports,
  sellRefundLamports as sellRefundFn,
} from "@/lib/amm/cpmm";
import { lpPreview } from "@/lib/amm/lp";
import { toast } from "sonner";
import { OrderBookDepth } from "@/components/OrderBookDepth";
const LivePriceChartPanel = dynamic(
  () =>
    import("@/components/LivePriceChartPanel").then(
      (m) => m.LivePriceChartPanel
    ),
  { ssr: false }
);
...
function dbRowToMarketDetails(row: any): MarketDetails | null {
  if (!row) return null;
  const catIdx =
    CATEGORIES.indexOf(row.category) >= 0
  ...
  const endTsVal = row.endTs
    ? Math.floor(new Date(row.endTs).getTime() / 1000)
    : FALLBACK_END;
  const resolveTsVal = row.resolveTs
    ? Math.floor(new Date(row.resolveTs).getTime() / 1000)
    : endTsVal + 3600;
  return {
...
function ProbabilityChart({ data }: { data: number[] }) {
  ...
  const pts = data.map((val, idx) => {
    ...
    const y =
      height -
      padding -
      (Math.max(0, Math.min(100, val)) / 100) * (height - padding * 2);
    return { x, y, val };
  });

  // Monotone (Catmull-Rom → bezier) smooth line through the points.
  const linePath = (() => {
  ...
  return (
    <div className="w-full bg-cream border border-hairline rounded-[8px] p-4 space-y-3 select-none">
      ...
      <div className="relative">
        ...
        >
          <defs>
            <linearGradient id="probChartArea" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor={lineColor} stopOpacity="0.28" />
              <stop offset="100%" stopColor={lineColor} stopOpacity="0.02" />
            </linearGradient>
            <linearGradient id="probChartStroke" x1="0" y1="0" x2="1" y2="0">
          ...
          {[25, 50, 75].map((lvl) => {
            ...
            const nearLast = Math.abs(last.val - lvl) <= 6;
            return (
              <g key={lvl}>
                <line
                  x1={padding}
                  ...
                  strokeWidth="1"
                  strokeDasharray={nearLast ? "0" : "4 4"}
                />
                <text
                  x={padding - 7}
                  y={y + 3}
          ...
          {/* Area fill */}
          <path d={areaPath} fill="url(#probChartArea)" />

          {/* Smooth line */}
          <path
            d={linePath}
            fill="none"
            stroke="url(#probChartStroke)"
            strokeWidth="2.5"
            strokeLinecap="round"
            ...
            if (!isLast) {
              return (
                <circle
                  ...
                  cy={pt.y}
                  r="3"
                  className="fill-[var(--color-cream)] stroke-cyan"
                  strokeWidth="1.5"
                />
              );
            }
...
export default function MarketDetailPage({
  ...
  const [lpOption, setLpOption] = useState<"balanced" | "yes" | "no">(
    "balanced"
  );
  ...
  const probHistory = useRef<number[]>(
    initialHistory && initialHistory.length > 1
      ? initialHistory
          .map((p) => Math.max(1, Math.min(99, Math.round(Number(p.yesPct)))))
          .slice(-30)
      : [50]
  );
  ...
  const assertSufficientBalance = async (
    ...
    const balLamports = await connection
    ...
    if (balSol < requiredSol + bufferSol) {
      toast.error(
        `Insufficient SOL for ${action}. You have ${balSol.toFixed(
          4
        )} SOL but need ~${(requiredSol + bufferSol).toFixed(4)}. ` +
  ...
  const isBlockhashError = (e: unknown): boolean => {
    ...
    return (
      ...
      /not found|expired|too old|unavailable/i.test(msg)
    );
  };
  ...
  const sendTxWithBlockhashRetry = async (builder: {
  ...
  const fetchLpInfo = useCallback(async () => {
    ...
    try {
      const res = await fetch(
        `/api/markets/${marketPda.toBase58()}/liquidity${q}`,
        { cache: "no-store" }
      );
      if (!res.ok) return;
      ...
      /* non-critical — LP tab degrades to the deposit form */
    }
  ...
  const fetchUserBalances = async () => {
    ...
    try {
      ...
      try {
        const res = await fetch(
          `/api/user/positions?wallet=${wallet.publicKey.toBase58()}`
        );
        const data = await res.json();
        if (data?.ok && data.positions) {
          const mktPosList = data.positions.filter(
            (p: any) => p.marketPubkey === marketB58
          );
          for (const p of mktPosList) {
            if (p.side === "YES" && p.shares > 0)
              yesBal = Math.max(yesBal, p.shares);
            if (p.side === "NO" && p.shares > 0)
              noBal = Math.max(noBal, p.shares);
          }
        }
      } catch {
  ...
  const syncMarketToDb = useCallback(
    async (details: MarketDetails) => {
      try {
  ...
  const readFreshAccount = async (
    ...
    for (let i = 0; i < 12; i++) {
      try {
        const acc = (await program.account.market.fetch(
          marketPda
  ...
  const syncTradeToDb = async (
    ...
    try {
      ...
      if (side === "YES") {
        newYesPool = Math.max(
          0.001,
          isBuy ? currentYes + amountSol : currentYes - amountSol
        );
      } else {
        newNoPool = Math.max(
          0.001,
          isBuy ? currentNo + amountSol : currentNo - amountSol
        );
      }
        ...
        try {
          ...
          if (!cachedMarket) {
            ...
            if (res2.ok) {
              ...
              if (json2.markets) {
                cachedMarket = json2.markets.find(
                  (c: any) =>
                    c.marketPubkey === id ||
  ...
  const handlePlaceLimitOrder = async (isBuy: boolean = true) => {
    ...
    if (!isBuy) {
      const currentHoldings =
        targetSide === "YES" ? userYesBalance : userNoBalance;
      if (currentHoldings < targetQty) {
        toast.error(
          `Insufficient ${targetSide} shares to place limit sell order (have ${currentHoldings.toFixed(
            1
          )}, need ${targetQty})`
        );
        return;
    ...
    try {
      ...
      const orderId = new anchor.BN(orderIdBytes[0])
        .shln(32)
        .or(new anchor.BN(orderIdBytes[1]));
      ...
      let builder: any = program.methods.placeOrder(
        orderId,
        sideParam,
        isBuy,
        priceBps,
      ...
      setTxSig(sig);
      toast.success(
        `Limit ${isBuy ? "Buy Bid" : "Sell Ask"} placed! (Sig: ${sig.slice(
          0,
          8
        )}...)`
      );
  ...
  const handleFillOrder = async (orderAccount: any, fillQty: number) => {
    ...
    try {
      ...
      const mint = isYes
        ? getYesMintPda(marketPda, program.programId)
      ...
      const sig = await sendTxWithBlockhashRetry(
        program.methods.fillOrder(new anchor.BN(fillQty)).accounts(
          txAccounts({
      ...
      toast.success(
        `Filled ${fillQty} shares order! (Sig: ${sig.slice(0, 8)}...)`
      );
      fetchMarket();
      fetchUserOrders();
      fetchUserBalances();
  ...
  const handleCancelOrder = async (orderAccount: any) => {
    ...
    try {
      ...
      const sig = await sendTxWithBlockhashRetry(
        program.methods.cancelOrder().accounts(
          txAccounts({
            maker: wallet.publicKey,
            market: marketPda,
            order: orderAccount.publicKey,
            ...
            orderEscrow: getOrderEscrowPda(
              marketPda,
              wallet.publicKey,
              new anchor.BN(orderAccount.account.orderId),
              program.programId
            ),
            emergencyPause: getEmergencyPausePda(program.programId),
          })
      ...
      setUserOrders((prev) =>
        prev.filter((o) => o.publicKey.toBase58() !== cancelledKey)
      );
      let unmounted = false;
      void (async () => {
        for (let i = 0; i < 15; i++) {
          ...
          if (unmounted) return;
          const acc = await (program.account.order as any)
            .fetch(orderAccount.publicKey)
            .catch(() => null);
        ...
        for (const event of events) {
          if (event.name === "SharesPurchased") {
            const {
              ...
              newYesPool,
              newNoPool,
            } = event.data as unknown as SharesPurchasedEvent;
            ...
            const yesProbVal =
              total > 0 ? Math.round((yesP / total) * 100) : 50;
            ...
            const { winningOutcome, settledPrice } =
              event.data as unknown as MarketSettledEvent;
            items.push({
              signature: sig.signature,
              slot: sig.slot,
            ...
            const { claimer, payout } =
              event.data as unknown as RewardsClaimedEvent;
            items.push({
              signature: sig.signature,
              slot: sig.slot,
  ...
  useEffect(() => {
    ...
    const seededPubkey = (initialMarket as any)?.marketPubkey as
      | string
      | undefined;
    const seedMismatch =
      !!initialMarket &&
  ...
  const shareYesPct = Math.round(
    (totalLamports > 0 ? yesLamports / totalLamports : 0.5) * 100
  );
  ...
  const getFeedIdHexString = (
    feedId: number[] | Uint8Array | Buffer
  ): string => {
  ...
  const settledAtStr =
    settledAtNum > 0 ? new Date(settledAtNum * 1000).toISOString() : null;
  ...
  const quoteBuyCostLamports = (): bigint => {
    ...
    try {
      return buyCostLamports(
        { poolYes: poolYesBI, poolNo: poolNoBI, feeBps: fee },
        tradeSide,
        dyOutBI
      );
    } catch {
  ...
  const priceImpactPct =
    avgPerShareSol > 0 && marginalPerShareSol > 0
  ...
  const activeSharePriceSol =
    quantity > 0 ? tradeCost / quantity : sharePriceSol;
  const yesSharePriceSol =
    tradeSide === "YES" ? activeSharePriceSol : sharePriceSol;
  const noSharePriceSol =
    tradeSide === "NO" ? activeSharePriceSol : sharePriceSol;
  ...
  const handleBuy = async () => {
    ...
    if (balOk)
      console.info(
        `[perf] buy pre-popup checks: ${Math.round(performance.now() - t0)}ms`
      );
    if (!balOk) return;
    if (!deployed) {
      toast.error(
    ...
    try {
      ...
      console.info(
        `[perf] buy build ${Math.round(tSend - tBuild)}ms + rpc ${Math.round(
          performance.now() - tSend
        )}ms`
      );
      ...
      toast.error(
        `Purchase failed: ${getFriendlyErrorMessage(err)}${
          isBlockhashError(err) ? BLOCKHASH_HINT : ""
        }`
      );
    } finally {
      setSubmitting(false);
  ...
  const handleSell = async () => {
    ...
    if (balOk)
      console.info(
        `[perf] sell pre-popup checks: ${Math.round(performance.now() - t0)}ms`
      );
    if (!balOk) return;
    if (!deployed) {
      toast.error(
    ...
    try {
      ...
      const minProceedsLamports = new anchor.BN(
        (sellRefundTreasuryCapBI * 95n) / 100n
      );
      const sig = await sendTxWithBlockhashRetry(
        program.methods
  ...
  const handleProvideLiquidity = async () => {
    ...
    if (balOk)
      console.info(
        `[perf] LP pre-popup checks: ${Math.round(performance.now() - t0)}ms`
      );
    if (!balOk) return;
    if (!deployed) {
      toast.error(
    ...
    try {
      ...
      toast.error(
        `Liquidity provision failed: ${getFriendlyErrorMessage(err)}${
          isBlockhashError(err) ? BLOCKHASH_HINT : ""
        }`
      );
    } finally {
    ...
    <main className="mx-auto w-full max-w-[1240px] px-6 py-10">
      ...
      <div className="grid lg:grid-cols-12 gap-8 items-start">
        ...
        <section className="lg:col-span-8 space-y-8">
          ...
          <OrderBookDepth
            yesPoolLamports={market.yesPoolLamports.toNumber()}
            noPoolLamports={market.noPoolLamports.toNumber()}
