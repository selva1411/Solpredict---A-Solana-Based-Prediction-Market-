...
const StatTile3D = dynamic(
  () => import("@/components/dashboard/StatTile3D").then((m) => m.StatTile3D),
  { ssr: false }
);
...
function AdminPage() {
  ...
  const [targetPriceVal, setTargetPriceVal] = useState<number>(250.0);
  ...
  const [settlePrices, setSettlePrices] = useState<Map<string, number>>(
    new Map()
  );
  const [leaderboardFallback, setLeaderboardFallback] = useState<
    Array<{ wallet: string; totalWagered: number }>
  >([]);
  ...
  const settleFeedHex = (() => {
    if (
      !settleModal.isOpen ||
      !settleModal.market ||
      !isOracleSettleable(settleModal.market.account.oracleFeedId)
    )
      return null;
    return (
      "0x" +
      Array.from(settleModal.market.account.oracleFeedId)
  ...
  useEffect(() => {
    if (settlePriceQuery.data != null) {
      setSettleModal((prev) => ({
        ...prev,
        settlePrice: settlePriceQuery.data!.toFixed(6),
        isFetchingPrice: false,
        livePrice: settlePriceQuery.data,
      }));
    }
  }, [settlePriceQuery.data]);

  const [oracleFeedIdHex, setOracleFeedIdHex] = useState<string>(
  ...
  const getSettlePrice = (marketKey: string): number => {
    return settlePrices.get(marketKey) ?? 260.0;
  };
    ...
    try {
      ...
      try {
        ...
        const accounts = program.account as unknown as {
          emergencyPause: {
            fetch(pda: PublicKey): Promise<{ paused: boolean }>;
          };
        };
        const emergencyAcc = await accounts.emergencyPause
          .fetch(emergencyPda)
          .catch(() => null);
      ...
      const onChainMarkets = (await program.account.market
        .all()
        .catch(() => [])) as Market[];
      ...
      try {
        const userPositions = await program.account.userPosition
          .all()
          .catch(() => []);
        const distinctTraders = new Set(
          userPositions.map((pos) => pos.account.owner.toBase58())
        );
        ...
        if (leaderboardRes?.ok && leaderboardRes.leaderboard) {
          ...
          leaderboardRes.leaderboard.forEach((user: { wallet: string }) =>
            distinctTraders.add(user.wallet)
          );
        }
  ...
  const handleInitializeConfig = async () => {
    ...
    try {
      ...
      if (balance < 10_000_000) {
        toast.error(
          `Insufficient SOL: ${lamportsToSol(balance).toFixed(
            4
          )} SOL. Need at least 0.01 SOL.`
        );
        return;
      }
  ...
  const handleTransferAdmin = async () => {
    ...
    try {
      ...
      await sendWithRetry(
        program,
        program.methods.updateAdmin(newAdmin).accounts(
          txAccounts({
            admin: wallet.publicKey,
            config: configPda,
          })
  ...
  const handleApproveProposal = async (
    ...
    let onChainProposal;
    try {
      onChainProposal = await program.account.marketProposal.fetch(
        new PublicKey(proposal.proposalPubkey)
      );
    } catch {
      ...
      throw new Error(
        "Proposal account not found on-chain — it may have already been approved or rejected."
      );
    }
    ...
    const approveSig = await sendWithRetry(
      program,
      program.methods.approveMarket().accounts(
        txAccounts({
          admin: wallet.publicKey,
          config: configPda,
    ...
    if (!syncRes.ok) {
      ...
      throw new Error(
        `Market synced on-chain but DB cache failed: ${
          (errBody as { error?: string }).error ?? `HTTP ${syncRes.status}`
        }`
      );
    }
    ...
    if (!dbRes.ok)
      throw new Error(
        `Proposal approved on-chain but DB status update failed (HTTP ${dbRes.status})`
      );

    toast.success(
      seeded
        ? `Proposal approved — market seeded with ${seededTotalSol.toFixed(
            2
  ...
  const handleRejectProposal = async (proposal: {
    ...
    await sendWithRetry(
      program,
      program.methods.rejectMarket().accounts(
        txAccounts({
          admin: wallet.publicKey,
          config: configPda,
          proposal: new PublicKey(proposal.proposalPubkey),
    ...
    if (!dbRes.ok)
      throw new Error(
        `Proposal rejected on-chain but DB status update failed (HTTP ${dbRes.status})`
      );
  ...
  const handleCreateMarket = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isOracleCategory(category)) {
        ...
        if (hex.length !== 64) {
          throw new Error(
            "Oracle Feed ID must be a 32-byte hex string (64 characters)"
          );
        }
        for (let i = 0; i < 32; i++) {
      ...
      const endTs = new anchor.BN(
        Math.floor(Date.now() / 1000) + durationSecs + 30
      );
      ...
      try {
        await fetch("/api/sync/market", {
          ...
          body: JSON.stringify({
            marketPubkey: marketPda.toBase58(),
            marketId:
              typeof nextMarketId === "number"
                ? nextMarketId
                : (nextMarketId as any).toNumber(),
            question,
            description,
  ...
  const syncMarketStatusToDb = async (
    market: Market,
    status: "settled" | "cancelled" | "open",
  ...
  const handleMockSettle = async (market: Market, settlePrice: number) => {
    ...
    try {
      ...
      const targetPriceNorm =
        market.account.targetPrice.toNumber() /
        Math.pow(10, Math.abs(market.account.targetExpo));
      const winSide = settlePrice >= targetPriceNorm ? "YES" : "NO";
      const accInfo = await connection
        .getAccountInfo(market.publicKey)
        .catch(() => null);
      ...
      const configPda = getConfigPda(program.programId);
      const mockPriceUpdatePda = getMockPriceUpdatePda(
        wallet.publicKey,
        program.programId
      );

      const scaleMultiplier = Math.pow(10, Math.abs(market.account.targetExpo));
      const settlePriceScaled = new anchor.BN(
        Math.round(settlePrice * scaleMultiplier)
      );
      ...
      const settleBuilder = program.methods.settleMarket().accounts(
        txAccounts({
          admin: wallet.publicKey,
  ...
  const handleWarpPrice = async (market: Market) => {
    ...
    try {
      ...
      if (!accInfo) {
        toast.info(
          "Historical DB market — warp only works on markets live on this validator."
        );
        return;
      }
      const scaleMultiplier = Math.pow(10, Math.abs(market.account.targetExpo));
      const warpPrice =
        warpPrices.get(marketKey) ??
        market.account.targetPrice.toNumber() / scaleMultiplier;
      const warpPriceScaled = new anchor.BN(
        Math.round(warpPrice * scaleMultiplier)
      );
      const conf = new anchor.BN(0);
      const exponent = market.account.targetExpo;
      const publishTime = new anchor.BN(Math.floor(Date.now() / 1000));
      ...
      const priceUpdateBuilder = program.methods
        ...
        .accounts(
          txAccounts({
            payer: wallet.publicKey,
            priceUpdate: getMockPriceUpdatePda(
              wallet.publicKey,
              program.programId
            ),
          })
      ...
      await sendWithRetry(program, priceUpdateBuilder);
      toast.success(
        `Oracle price warped to ${warpPrice} for market #${market.account.marketId.toString()}`
      );
    } catch (err: unknown) {
      console.error(err);
  ...
  const handleCancelMarket = async (marketPda: PublicKey) => {
    ...
    try {
      ...
      const configPda = getConfigPda(program.programId);
      const cancelBuilder = program.methods.cancelMarket().accounts(
        txAccounts({
          admin: wallet.publicKey,
          config: configPda,
          market: marketPda,
      ...
      toast.success(
        "Market cancelled on-chain. Traders may claim full refunds."
      );
      if (targetMarket) {
        await syncMarketStatusToDb(targetMarket, "cancelled");
      }
  ...
  const handleWithdrawFees = async (market: Market) => {
    ...
    try {
      ...
      const withdrawBuilder = program.methods.withdrawFees().accounts(
        txAccounts({
          admin: wallet.publicKey,
          config: configPda,
          market: market.publicKey,
  ...
  const platformStats = useMemo(() => {
    ...
    markets.forEach((m) => {
      ...
      const onChainFee = m.account.feeCollected
        ...
        : 0;
      totalFeesCollectedLamports +=
        onChainFee > 0
          ? onChainFee
          : Math.round(volume * ((config?.feeBps || 200) / 10000));
  ...
  if (configLoading) {
    return (
      <div className="space-y-6 animate-pulse">
        <div className="h-10 bg-sheet border border-hairline rounded-[8px] w-1/3" />
        <div className="grid grid-cols-2 sm:grid-cols-3 xl:grid-cols-5 gap-6">
          {[...Array(5)].map((_, i) => (
            <div
              key={i}
              className="h-28 surface skeleton-shimmer rounded-[8px]"
            />
          ))}
        </div>
      </div>
  ...
  return (
    <main className="max-w-7xl mx-auto px-4 sm:px-6 py-6 sm:py-10">
      {settleModal.isOpen && settleModal.market && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ground/80 ">
          <div className="surface max-w-md w-full p-6 space-y-6 border-t-4 border-t-cyan relative shadow-2xl rounded-[8px]">
            <div className="absolute top-3 left-4 flex items-center space-x-2">
              <span className="w-2 h-2 rounded-[2px] bg-cyan animate-pulse" />
              <span className="text-[9px] font-mono tracking-widest text-cyan">
            ...
            <div className="pt-4 space-y-3 font-mono text-xs">
              <div className="space-y-1">
                ...
                <div className="text-[13px] font-bold text-ink">
                  {settleModal.market.account.question}
                </div>
              </div>

              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 border-y border-hairline/20 py-3">
                <div className="space-y-0.5">
                  <div className="text-[10px] text-ash uppercase">
                    Target Price:
                  </div>
                  <div className="text-[13px] font-bold text-magenta">
                    $
                    {(
                      settleModal.market.account.targetPrice.toNumber() /
                      Math.pow(
                        10,
                        Math.abs(settleModal.market.account.targetExpo)
                      )
                    ).toFixed(2)}
                  </div>
                </div>
                <div className="space-y-0.5">
                  ...
                  <div className="text-[13px] font-bold text-magenta">
                    {settleModal.market.account.comparison === 0
                      ? "Greater Than"
                      : "Less Than"}
                  </div>
                </div>
              ...
              <div className="space-y-2">
                <div className="flex justify-between items-center">
                  <span className="text-[10px] text-ash uppercase">
                    Settlement Price ($):
                  </span>
                  {settleModal.isFetchingPrice && (
                    <span className="text-[9px] text-cyan animate-pulse">
                      FETCHING REAL-TIME PYTH...
                    </span>
                  )}
                </div>
                <input
                  type="text"
                  value={
                    settleModal.settlePrice ? `$${settleModal.settlePrice}` : ""
                  }
                  readOnly
                  className="w-full bg-[var(--surface-1)] border border-hairline rounded-[4px] px-3 py-2 text-ink focus:outline-none focus:border-magenta focus:ring-1 focus:ring-magenta/30 py-2 px-3 text-[13px] font-mono border-hairline bg-ground/50 text-magenta cursor-not-allowed opacity-80"
              ...
              {(() => {
                ...
                const target =
                  settleModal.market.account.targetPrice.toNumber() /
                  Math.pow(10, Math.abs(settleModal.market.account.targetExpo));
                const isGreater = settleModal.market.account.comparison === 0;
                const yesWins = isGreater
                  ? enteredPrice > target
                  : enteredPrice < target;

                return (
                  <div className="bg-ground border border-hairline/20 p-3 rounded-[4px] text-center">
                    <div className="text-[9px] text-ash uppercase">
                      Projected Outcome:
                    </div>
                    <div className="mt-1 text-md font-bold tracking-widest font-mono">
                      ...
                      , outcome will be{" "}
                      <span className={yesWins ? "text-grass" : "text-magenta"}>
                        {yesWins ? "YES" : "NO"}
                      </span>
                    </div>
                  </div>
            ...
            <div className="flex items-center gap-3 pt-2">
              <button
                ...
                className="flex-1 text-[13px] py-2 uppercase border-2 border-ink text-ink hover:bg-yellow hover:text-ink-static snap cursor-pointer bg-transparent rounded-[4px]"
              >
                Cancel
              </button>
              <button
                type="button"
                disabled={
                  isNaN(parseFloat(settleModal.settlePrice)) ||
                  settlingId !== null
                }
                onClick={async () => {
                ...
                className="flex-1 bg-ink-fill text-white text-[13px] py-2 uppercase snap cursor-pointer disabled:opacity-50 rounded-[4px] font-bold"
              >
      ...
      {manualSettleModal.isOpen && manualSettleModal.market && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-ground/80 ">
          <div className="surface max-w-md w-full p-6 space-y-6 border-t-4 border-t-magenta relative shadow-2xl rounded-[8px]">
            <div className="absolute top-3 left-4 flex items-center space-x-2">
              <span className="w-2 h-2 rounded-[2px] bg-magenta animate-pulse" />
              <span className="text-[9px] font-mono tracking-widest text-magenta bg-magenta/10 px-1.5 py-0.5 rounded-[4px] border border-magenta/20 font-bold">
                MANUAL SETTLEMENT
              </span>
            </div>
            ...
            {!manualSettleModal.showSecondaryConfirm ? (
              ...
              <div className="space-y-6 pt-4">
                <div className="space-y-1 text-left">
                  <div className="text-[10px] text-ash font-mono uppercase">
                    Question:
                  </div>
                  <div className="text-[13px] font-bold text-ink font-display">
                    {manualSettleModal.market.account.question}
                  </div>
                </div>

                {manualSettleModal.market.account.description && (
                  <div className="space-y-1 bg-ground p-3 border border-hairline/15 rounded-[4px] text-left">
                    <div className="text-[10px] text-ash font-mono uppercase">
                      Winning Condition:
                    </div>
                    <div className="text-[11px] text-ash leading-relaxed font-mono">
                      {manualSettleModal.market.account.description}
                    </div>
                  </div>
                )}

                <div className="grid grid-cols-2 gap-4 text-center font-mono text-xs border-y border-hairline/20 py-3 bg-ground/30 rounded-[4px]">
                  <div className="space-y-0.5">
                    <div className="text-[10px] text-ash uppercase">
                      YES Pool:
                    </div>
                    <div className="text-[13px] font-bold text-grass">
                      {lamportsToSol(
                        manualSettleModal.market.account.yesPoolLamports
                      ).toFixed(2)}{" "}
                      SOL
                    </div>
                  </div>
                  <div className="space-y-0.5">
                    ...
                    <div className="text-[13px] font-bold text-magenta">
                      {lamportsToSol(
                        manualSettleModal.market.account.noPoolLamports
                      ).toFixed(2)}{" "}
                      SOL
                    </div>
                  </div>
                ...
                <div className="space-y-3">
                  <div className="text-[10px] text-ash font-mono uppercase text-center">
                    Select Winner:
                  </div>
                  <div className="grid grid-cols-2 gap-4">
                    <button
                      type="button"
                      onClick={() =>
                        setManualSettleModal((prev) => ({
                          ...prev,
                          outcome: 1,
                          showSecondaryConfirm: true,
                        }))
                      }
                      className="py-3 font-mono font-bold uppercase rounded border-2 border-ink bg-yes-fill text-ink snap cursor-pointer text-center"
                    >
                      [ YES WON ]
                    </button>
                    <button
                      ...
                      className="py-3 font-mono font-bold uppercase rounded border-2 border-no bg-cream text-ink snap cursor-pointer text-center"
                    >
                      [ NO WON ]
                    </button>
                  </div>
                ...
                <button
                  ...
                  className="w-full text-[13px] py-2 uppercase border-2 border-ink text-ink hover:bg-yellow hover:text-ink-static snap cursor-pointer bg-transparent rounded-[4px]"
                >
                  Cancel
                </button>
              </div>
              ...
              <div className="space-y-6 pt-4 font-mono text-xs text-center">
                <div className="text-[13px] font-bold text-magenta uppercase tracking-wider flex items-center justify-center gap-2">
                  <AlertTriangle className="w-5 h-5 animate-pulse" /> EXTREME
                  WARNING
                </div>

                <div className="space-y-4 text-ash leading-relaxed text-[11px] bg-ground p-4 border border-magenta/30 rounded-[4px] text-left">
                  <p>
                    You are about to settle this market with{" "}
                    <span
                      className={
                        manualSettleModal.outcome === 1
                ...
                <div className="flex items-center gap-3">
                  ...
                  <button
                    ...
                    className="flex-1 bg-magenta text-white dark:text-ink-static text-[13px] py-2 uppercase snap cursor-pointer disabled:opacity-50 font-bold rounded-[4px]"
                  >
      ...
      {config && wallet?.publicKey && !isWalletAdmin && (
        <motion.div
          ...
          initial="hidden"
          animate="visible"
          className="p-4 sm:p-5 rounded-[8px] bg-magenta/10 border border-magenta/30 text-ink space-y-2 font-mono text-xs shadow-lg"
        >
          <div className="flex items-center space-x-2 text-magenta font-bold text-[13px]">
            <AlertTriangle className="w-5 h-5 shrink-0 animate-pulse text-magenta" />
            <span>ADMIN KEY MISMATCH</span>
          </div>
          <p className="text-ink-soft leading-relaxed font-sans text-xs">
            Your connected browser wallet{" "}
            <code className="bg-cream px-1.5 py-0.5 rounded-[4px] text-ink border border-hairline font-mono">
              {wallet.publicKey.toBase58()}
            </code>{" "}
            is not the on-chain Administrator authority. The on-chain Config PDA
            was initialized by key{" "}
            <code className="bg-cream px-1.5 py-0.5 rounded-[4px] text-magenta border border-hairline font-mono">
              {config.admin.toBase58()}
            </code>
            .
          </p>
          <div className="text-[11px] text-ink-soft pt-1 font-sans">
            <strong>
              To execute admin actions (Create Market, Settle, Cancel, Withdraw
              Fees):
            </strong>
            <ul className="list-disc list-inside mt-1 space-y-0.5">
              <li>
                Connect your wallet using authority key{" "}
                <code className="font-mono bg-cream border border-hairline px-1.5 py-0.5 text-ink rounded-[4px]">
                  {config.admin.toBase58()}
                </code>{" "}
                (or import{" "}
                <code className="font-mono bg-cream border border-hairline px-1.5 py-0.5 text-ink rounded-[4px]">
              ...
              <li>
                ...
                in terminal to reset localnet, then click{" "}
                <strong>Initialize Config PDA</strong> with your active browser
                wallet.
              </li>
            </ul>
          </div>
      ...
      {!config ? (
        <motion.section
          ...
          initial="hidden"
          animate="visible"
          className="surface p-8 space-y-6 rounded-[8px] border-t-4 border-t-magenta"
        >
          <div className="flex items-center space-x-3 text-magenta">
            <AlertTriangle className="w-6 h-6 animate-pulse" />
            <h2 className="text-[21px] font-bold font-display uppercase tracking-wider">
              Config PDA Not Found
            </h2>
          </div>
          <p className="text-xs text-ash leading-relaxed">
            The platform-wide config singleton must be initialized once before
            markets can be created. The key initialized here will serve as the
            master Administrator authority.
          </p>
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 items-end max-w-md">
            <div className="space-y-2">
              <label className="text-xs font-mono text-ash font-bold">
              ...
              <input
                ...
                className="w-full bg-[var(--surface-1)] border border-hairline rounded-[4px] px-3 py-2 text-ink focus:outline-none focus:border-cyan focus:ring-1 focus:ring-cyan/30 text-[13px] py-2 px-3 border-hairline"
              />
            </div>
            <button
              onClick={handleInitializeConfig}
              className="bg-ink-fill text-white py-2 text-xs w-full sm:w-auto snap rounded-[4px] font-bold px-4"
            >
        ...
        >
          <StatTile3D
            label="Total Volume"
            value={platformStats.totalVolume.toFixed(1)}
            unit="SOL"
            icon={BarChart3}
            delay={0}
          />
          <StatTile3D
            label="Open Markets"
            value={String(platformStats.openMarketsCount)}
            unit="QTY"
            icon={TrendingUp}
            accent="green"
            delay={0.05}
          ...
          <StatTile3D
            ...
            value={String(platformStats.settledMarketsCount)}
            unit="QTY"
            accent="neutral"
            delay={0.1}
          />
          ...
          <StatTile3D
            ...
            value={platformStats.totalFeesCollected.toFixed(4)}
            unit="SOL"
            delay={0.2}
            useSplitFlap={false}
          />
        ...
        <>
          ...
          <div className="flex gap-1 mb-8 border-b border-hairline pb-1 overflow-x-auto no-scrollbar">
            {SECTIONS.map((tab) => (
              <button
                ...
                className={`px-4 py-2 text-[13px] font-medium capitalize transition-all border-b-2 -mb-[1px] whitespace-nowrap rounded-t-[4px] ${
                  activeAdminSection === tab
                    ? "text-ink border-ink bg-cream"
          ...
          {activeAdminSection === "overview" && (
            <>
                ...
                >
                  <div className="divide-y divide-hairline/20">
                    {needsActionMarkets.map((m) => {
                      ...
                      const isOracle = isOracleSettleable(
                        m.account.oracleFeedId
                      );
                      return (
                        <div
                          key={marketKey}
                          className="py-3 sm:py-4 flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4"
                        >
                          <div className="space-y-1 min-w-0">
                            ...
                            <div className="text-[10px] text-ash font-mono flex flex-wrap items-center gap-2">
                              <span>
                                {getCategoryString(m.account.category)}
                              </span>
                              {isOracle ? (
                                <span className="text-cyan font-bold inline-flex items-center gap-0.5">
                                  <Zap className="w-3 h-3" /> ORACLE
                                </span>
                              ) : (
                                <span className="text-magenta font-bold inline-flex items-center gap-0.5">
                                  <Gavel className="w-3 h-3" /> MANUAL
                                </span>
                              )}
                            </div>
                          </div>
                          <div className="flex items-center gap-2 shrink-0">
                            <button
                              ...
                              className="bg-ink-fill text-white text-xs py-1.5 px-4 snap cursor-pointer disabled:opacity-50 rounded-[4px] font-bold"
                            >
                              {settlingId === marketKey
                                ? "Settling..."
                                : "Settle"}
                            </button>
                          </div>
              ...
              <DashboardSection
                title="Audit Trail Actions"
                icon={History}
                delay={0.15}
              >
                <div className="surface overflow-hidden hover-lift rounded-[8px]">
                  {activityLoading ? (
                    ...
                    <div className="overflow-x-auto scrollbar-thin">
                      <table className="w-full text-left border-collapse min-w-[500px]">
                        <thead>
                          <tr className="border-b border-hairline/30 text-[10px] font-mono uppercase tracking-widest text-ash bg-sheet">
                            ...
                            <th className="py-3 sm:py-4 px-3 sm:px-6 hidden sm:table-cell">
                              Detail
                            </th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-hairline/10 font-mono text-xs">
                          {adminActivity.map((item, idx) => {
                            ...
                            if (item.type === "CREATE")
                              ...
                              tagClass =
                                "bg-yellow/20 text-ink border border-yellow/40";

                            return (
                              <tr
                                key={idx}
                                className="table-row-3d hover:bg-sheet transition-colors"
                              >
                                <td className="py-3 sm:py-4 px-3 sm:px-6 text-ash">
                                  {item.timeStr}
                                </td>
                                <td className="py-3 sm:py-4 px-3 sm:px-6">
                                  <span
                                    className={`px-2 py-0.5 rounded-[4px] text-[10px] font-bold uppercase ${tagClass}`}
                                  >
                                    {item.type}
                                  </span>
                                </td>
                                <td className="py-3 sm:py-4 px-3 sm:px-6 text-ink max-w-[140px] sm:max-w-sm truncate font-bold">
                                  {item.question}
                                </td>
                                <td className="py-3 sm:py-4 px-3 sm:px-6 text-ash hidden sm:table-cell">
                                  {item.details}
                                </td>
                              </tr>
                            );
            ...
            <div className="grid grid-cols-1 xl:grid-cols-3 gap-6 sm:gap-8">
              ...
              <div className="space-y-4 sm:space-y-6">
                <div className="flex items-center space-x-2">
                  ...
                  <h2 className="text-[15px] sm:text-[21px] font-bold font-display uppercase tracking-wider font-bold">
                    Create Contract
                  </h2>
                </div>

                <div className="surface p-4 sm:p-6 space-y-4 hover-lift rounded-[8px] border-t-4 border-t-cyan">
                  <form onSubmit={handleCreateMarket} className="space-y-4">
                    <div className="space-y-1.5">
                      <label className="text-xs font-bold text-ash">
                        Question Text
                      </label>
                      <input
                        type="text"
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div className="space-y-1.5">
                        ...
                        <select
                          ...
                          className="w-full bg-[var(--surface-1)] border border-hairline rounded-[4px] px-3 py-2 text-ink focus:outline-none focus:border-cyan focus:ring-1 focus:ring-cyan/30 text-xs bg-sheet border-hairline"
                        >
                    ...
                    {isOracleCategory(category) ? (
                      <>
                        <div className="space-y-1.5">
                          <label className="text-xs font-bold text-ash">
                            Asset (auto-fills feed ID & target)
                          </label>
                          <select
                          ...
                          >
                            ...
                            {Object.entries(PYTH_FEED_REGISTRY).map(
                              ([key, entry]) => (
                        ...
                        <div className="space-y-1.5">
                          ...
                          <input
                            ...
                            className="w-full bg-[var(--surface-1)] border border-hairline rounded-[4px] px-3 py-2 text-ink focus:outline-none focus:border-cyan focus:ring-1 focus:ring-cyan/30 text-xs border-hairline font-mono text-cyan"
                          />
                        </div>
                        <div className="grid grid-cols-2 gap-4">
                          <div className="space-y-1.5">
                            ...
                            <input
                              ...
                              onChange={(e) =>
                                setTargetPriceVal(Number(e.target.value))
                              }
                              className="w-full bg-[var(--surface-1)] border border-hairline rounded-[4px] px-3 py-2 text-ink focus:outline-none focus:border-cyan focus:ring-1 focus:ring-cyan/30 text-xs border-hairline"
                            />
                          </div>
                          <div className="space-y-1.5">
                            ...
                            <input
                              ...
                              value={durationSecs}
                              onChange={(e) =>
                                setDurationSecs(Number(e.target.value))
                              }
                              className="w-full bg-[var(--surface-1)] border border-hairline rounded-[4px] px-3 py-2 text-ink focus:outline-none focus:border-cyan focus:ring-1 focus:ring-cyan/30 text-xs border-hairline"
                            />
                            <p className="text-[10px] text-ash">
                              Minimum: 30s (program requires end &ge;60s out)
                            </p>
                          </div>
                        </div>
                      </>
                      ...
                      <>
                        <div className="space-y-1.5 bg-cyan/5 border border-cyan/20 p-3 rounded-[4px] font-mono text-[10px] text-cyan leading-normal text-left">
                          This market will be manually settled by the admin. Use
                          the Description field to clearly explain what the YES
                          and NO outcomes mean (e.g. 'YES if the Giants win, NO
                          if they lose or the game is cancelled').
                        </div>
                        <div className="space-y-1.5">
                          <label className="text-xs font-bold text-ash">
                            Duration (Secs)
                        ...
                        <div className="grid grid-cols-2 gap-3 pt-1">
                          <div className="space-y-1.5">
                            <label className="text-xs font-bold text-grass">
                              YES Pool Seed (SOL)
                            </label>
                            <input
                              type="number"
                              ...
                              onChange={(e) =>
                                setInitialYesPoolSol(
                                  Math.max(0.1, Number(e.target.value))
                                )
                              }
                              className="w-full bg-[var(--surface-1)] border border-grass/40 rounded-[4px] px-3 py-2 text-ink focus:outline-none focus:border-grass text-xs font-mono"
                            />
                          </div>
                          <div className="space-y-1.5">
                            <label className="text-xs font-bold text-magenta">
                              NO Pool Seed (SOL)
                            </label>
                            <input
                              type="number"
                              ...
                              className="w-full bg-[var(--surface-1)] border border-magenta/40 rounded-[4px] px-3 py-2 text-ink focus:outline-none focus:border-magenta text-xs font-mono"
                            />
                          </div>
                        </div>
                        <div className="text-[10px] font-mono text-magenta flex justify-between px-1">
                          <span>
                            Total Seed:{" "}
                            {(initialYesPoolSol + initialNoPoolSol).toFixed(1)}{" "}
                            SOL
                          </span>
                          <span>
                            Starting Odds:{" "}
                            {(
                              (initialYesPoolSol /
                                Math.max(
                                  0.1,
                                  initialYesPoolSol + initialNoPoolSol
                                )) *
                              100
                            ).toFixed(0)}
                            % YES /{" "}
                    ...
                    <button
                      type="submit"
                      className="w-full bg-ink-fill text-white text-xs py-2.5 mt-4 snap rounded-[4px] font-bold"
                    >
                      Deploy Market PDA (
                      {(initialYesPoolSol + initialNoPoolSol).toFixed(1)} SOL
                      Custom Liquidity)
                    </button>
                  </form>
                </div>
                      ...
                      <table className="w-full text-left border-collapse min-w-[600px]">
                        <thead>
                          <tr className="border-b border-hairline/30 text-[10px] font-mono uppercase tracking-widest text-ash bg-sheet">
                            ...
                            <th className="py-3 sm:py-4 px-3 sm:px-6 text-right">
                              Volume
                            </th>
                        ...
                        <tbody className="divide-y divide-hairline/10 font-mono text-xs">
                          {markets.map((m) => {
                            const status = getMarketStatusString(
                              m.account.status,
                              m.account.endTs
                            );
                            const marketKey = m.publicKey.toBase58();
                            const yesPool = lamportsToSol(
                              m.account.yesPoolLamports
                            );
                            ...
                            return (
                              ...
                              >
                                <td className="py-3 sm:py-4 px-3 sm:px-6 text-ash">
                                  #{m.account.marketId.toString()}
                                </td>
                                <td className="py-3 sm:py-4 px-3 sm:px-6 text-ink max-w-[140px] sm:max-w-xs truncate font-bold">
                                  {m.account.question}
                                </td>
                                <td className="py-3 sm:py-4 px-3 sm:px-6">
                                  {(() => {
                                    const now = Math.floor(Date.now() / 1000);
                                    const isPast =
                                      m.account.resolveTs.toNumber() <= now ||
                                      m.account.endTs.toNumber() <= now;
                                    const feedIdNonZero =
                                      m.account.oracleFeedId.some(
                                        (b) => b !== 0
                                      );
                                    if (
                                      (status === "Open" ||
                                      ...
                                      if (feedIdNonZero) {
                                        return (
                                          <span className="px-2 py-0.5 rounded-[4px] text-[10px] font-bold bg-cyan/15 text-cyan border border-cyan/20 inline-flex items-center gap-1">
                                        ...
                                        return (
                                          <span className="px-2 py-0.5 rounded-[4px] text-[10px] font-bold bg-magenta/15 text-magenta border border-magenta/20 inline-flex items-center gap-1">
                                            <Gavel className="w-3 h-3 text-magenta" />{" "}
                                            MANUAL SETTLE
                                          </span>
                                        );
                                      }
                                      ...
                                      <span
                                        className={`px-2 py-0.5 rounded-[4px] text-[10px] font-bold ${
                                          status === "Open"
                                            ...
                                            : status === "Settled"
                                            ? "bg-sheet text-ash border border-hairline"
                                            : "bg-magenta/15 text-magenta border border-magenta/20"
                                        }`}
                                      >
                                        {status}
                                      </span>
                                ...
                                <td className="py-3 sm:py-4 px-3 sm:px-6 text-right text-ink num">
                                ...
                                <td className="py-3 sm:py-4 px-3 sm:px-6 text-center">
                                    ...
                                    <div className="flex items-center justify-center gap-2">
                                      {isOracleSettleable(
                                        ...
                                        <button
                                          ...
                                          className="px-2.5 py-1 bg-cyan/15 hover:bg-cyan/25 border border-cyan/40 rounded-[4px] text-[9px] text-cyan cursor-pointer font-bold"
                                      ...
                                      <button
                                        disabled={settlingId !== null}
                                        onClick={() =>
                                          handleSettleButtonClick(m)
                                        }
                                        className="px-2.5 py-1 bg-grass hover:bg-grass/70 text-ink-static border-2 border-ink-static rounded-[4px] text-[9px] cursor-pointer font-bold snap"
                                      >
                                        Settle
                                      </button>
                                      <button
                                        ...
                                        className="px-2.5 py-1 bg-magenta hover:bg-magenta/80 text-white dark:text-ink-static border border-hairline rounded-[4px] text-[9px] cursor-pointer font-bold"
                                      >
                                        Cancel
                                      </button>
                                    </div>
                                  ...
                                  {status === "Settled" && (
                                    <div className="flex items-center justify-center">
                                      {m.account.feeWithdrawn ? (
                                        <span className="text-grass text-[10px] font-bold uppercase">
                                          Withdrawn
                                        </span>
                                      ) : (
                                        <button
                                          onClick={() => handleWithdrawFees(m)}
                                          className="px-2.5 py-1 bg-yellow/20 hover:bg-yellow/40 border border-yellow/40 rounded-[4px] text-[9px] text-ink font-bold cursor-pointer uppercase"
                                        >
                                          Withdraw Fees (
                                          {lamportsToSol(
                                            m.account.feeCollected
                                          ).toFixed(3)}{" "}
                                          SOL)
                                        </button>
                                      )}
                                  ...
                                  {status === "Cancelled" && (
                                    <span className="text-ash text-[10px] italic">
                                      Refunds enabled
                                    </span>
                                  )}
                                </td>
          ...
          {/* USERS TAB */}
          {activeAdminSection === "users" && <UsersSection />}

          {/* DISPUTES TAB */}
          {activeAdminSection === "disputes" && <DisputesSection />}
          ...
          {activeAdminSection === "config" && (
            <motion.section
              ...
              initial="hidden"
              animate="visible"
              className="surface p-8 space-y-6 rounded-[8px] border-t-4 border-t-grass"
            >
              <div className="flex items-center justify-between">
                <h2 className="text-[21px] font-bold font-display uppercase tracking-wider text-ink">
                  Platform Configuration
                </h2>
                <div className="flex items-center gap-3">
                  {paused && (
                    <span className="inline-flex items-center gap-2 text-xs font-mono uppercase tracking-widest text-magenta bg-magenta/10 border border-magenta/30 rounded-[4px] px-3 py-1.5">
                      <span className="h-1.5 w-1.5 rounded-[2px] bg-magenta animate-pulse" />
                      Emergency Paused
                    </span>
                  )}
                  <button
                    onClick={handleInitializeConfig}
                    className="bg-ink-fill text-white py-2 text-xs snap rounded-[4px] font-bold px-4"
                  >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div className="bg-sheet rounded-[8px] p-4 space-y-2 border border-hairline">
                  <div className="text-[10px] font-mono uppercase tracking-widest text-ash">
                    Admin Authority
                  </div>
                  <div className="text-[13px] font-mono text-ink break-all">
                    {config?.admin.toBase58() || "—"}
                  </div>
                </div>
                <div className="bg-sheet rounded-[8px] p-4 space-y-2 border border-hairline">
                  ...
                  <div className="text-[13px] font-mono text-ink num">
                    {config?.feeBps || 0} bps ({(config?.feeBps || 0) / 100}%)
                  </div>
                </div>
                <div className="bg-sheet rounded-[8px] p-4 space-y-2 border border-hairline">
                  <div className="text-[10px] font-mono uppercase tracking-widest text-ash">
              ...
              <div className="pt-4 border-t border-hairline">
                <button
                  ...
                  className="bg-ink-fill text-white text-xs py-2 px-6 snap cursor-pointer disabled:opacity-50 rounded-[4px] font-bold"
                >
              ...
              <div className="pt-4 border-t border-hairline space-y-3">
                ...
                <p className="text-xs text-ash leading-relaxed">
                  Transfer the platform admin role to another wallet. Only the
                  current admin can do this. The change takes effect immediately
                  and cannot be undone by the old admin.
                </p>
                <div className="flex flex-col sm:flex-row gap-3">
                  <input
                    ...
                    className="flex-1 bg-sheet border border-hairline rounded-[4px] px-3 py-2 text-xs font-mono text-ink focus:outline-none focus:border-cyan"
                  />
                  <button
                    onClick={handleTransferAdmin}
                    disabled={transferringAdmin || !isWalletAdmin}
                    className="bg-ink-fill text-white text-xs py-2 px-6 snap cursor-pointer disabled:opacity-50 disabled:cursor-not-allowed rounded-[4px] font-bold"
                  >
                    {transferringAdmin ? "Transferring..." : "Transfer Admin"}
                  </button>
                </div>
                {!isWalletAdmin && (
                  <p className="text-[11px] text-magenta/90">
              ...
              <div className="flex gap-2 pt-3">
                <button
                  ...
                  className="flex-1 text-xs py-2 px-4 rounded-[4px] border-2 border-magenta text-magenta hover:bg-magenta/10 transition-colors cursor-pointer disabled:opacity-50 font-bold"
                >
                  Emergency Pause
                </button>
                <button
                  ...
                  disabled={pausing || !paused}
                  className="flex-1 text-xs py-2 px-4 rounded-[4px] border-2 border-grass text-grass hover:bg-grass/10 transition-colors cursor-pointer disabled:opacity-50 font-bold"
                >
                  Unpause Program
                </button>
              </div>
