...
export default function AdminTreasuryPage() {
  ...
  const handleRecordWithdrawal = async () => {
    ...
    const recipient = window
      .prompt("Recipient wallet for the withdrawal (defaults to admin):", "")
      ?.trim();
    ...
    try {
      ...
      window.alert(
        "Failed to record withdrawal. Check that the transaction confirmed."
      );
    } finally {
      setRecording(false);
    }
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center gap-3 justify-between">
        <div className="flex items-center gap-3">
          <DollarSign className="w-5 h-5 text-grass" />
      ...
      {loading ? (
        ...
        <>
            ...
            {[
              {
                ...
                value: `${(data?.totalFeeCollectedSol || 0).toFixed(4)} SOL`,
                icon: DollarSign,
                color: "text-grass",
                bg: "bg-grass/10",
                bar: "border-t-grass",
              },
              {
                ...
                value: `${(data?.totalFeeWithdrawnSol || 0).toFixed(4)} SOL`,
                icon: ArrowDownRight,
                color: "text-cyan",
                bg: "bg-cyan/10",
                bar: "border-t-cyan",
              ...
              {
                label: "Pending Fees",
                value: `${(data?.pendingFeesSol || 0).toFixed(4)} SOL`,
                icon: Wallet,
                color: "text-inkblue",
                bg: "bg-inkblue/10",
                bar: "border-t-inkblue",
              },
              {
                label: "Trade Volume",
                value: `${(data?.totalTradeVolume || 0).toFixed(2)} SOL`,
                icon: TrendingUp,
                color: "text-magenta",
                bg: "bg-magenta/10",
                bar: "border-t-magenta",
              },
            ].map((card, i) => (
              <motion.div
                key={card.label}
                ...
                className={`rounded-[8px] bg-cream border border-hairline border-t-4 ${card.bar} p-5`}
              >
                <div className="flex items-start justify-between mb-3">
                  <p className="text-xs text-ash uppercase tracking-wider">
                    {card.label}
                  </p>
                  <div className={`p-2 rounded-[8px] ${card.bg}`}>
          ...
          <div className="rounded-[8px] bg-cream border border-hairline p-6">
            ...
            <p className="text-xs text-ash mb-4">
              Execute the on-chain withdraw (Admin Console → Markets → Withdraw
              Fees), then record it here so the ledger stays accurate.
            </p>
            <div className="flex items-center gap-3">
              <div className="flex-1 px-4 py-3 rounded-[8px] bg-sheet border border-hairline">
                <p className="text-xs text-ash">Available for withdrawal</p>
                <p className="text-[21px] font-bold font-mono text-ink mt-0.5">
                  {(data?.pendingFeesSol || 0).toFixed(4)} SOL
                </p>
              </div>
              <button
                className="flex items-center gap-1.5 px-4 py-2 text-xs font-bold rounded-[4px] bg-ink-fill text-white hover:bg-ink-fill/90 transition-all disabled:opacity-50 cursor-pointer whitespace-nowrap active:scale-97"
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      {w.signature ? (
                        <a
                          ...
                          target="_blank"
                          rel="noreferrer"
                          className="inline-flex items-center gap-1 text-[10px] font-mono text-inkblue hover:text-cyan"
                        >
