...
export function AirdropSolButton() {
    ...
    try {
      ...
      toast.error(
        err instanceof Error
          ? err.message
          : "Airdrop failed. Is the local validator running?"
      );
    } finally {
      setBusy(false);
    }
  ...
  return (
    <button
      onClick={onAirdrop}
      disabled={busy}
      className="flex items-center gap-1.5 px-3 py-1.5 rounded-[4px] text-[10px] font-bold uppercase tracking-wider border-2 border-magenta/40 text-magenta hover:bg-magenta/10 transition-colors disabled:opacity-50 cursor-pointer snap"
