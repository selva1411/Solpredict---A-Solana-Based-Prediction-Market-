...
function FlipUnit({
  ...
  return (
    <div className="flex flex-col items-center">
      <div
        className={`relative overflow-hidden rounded-[4px] bg-cream border border-hairline-2 font-mono font-bold text-inkblue flex items-center justify-center ${
      ...
      {!compact && (
        <span className="text-[9px] uppercase tracking-wider text-ash mt-1">
          {label}
        </span>
      )}
    </div>
...
export const FlipCountdown = React.memo(function FlipCountdown({
  endTs,
  compact = false,
}: FlipCountdownProps) {
  ...
  if (parts.ended) {
    return (
      <span className="text-xs font-mono font-semibold text-ash">
        Trading ended
      </span>
    );
  }
  ...
  return (
    <div className={`flex items-center ${compact ? "gap-1" : "gap-2"}`}>
      {parts.days > 0 && (
        <FlipUnit value={parts.days} label="Days" compact={compact} />
      )}
