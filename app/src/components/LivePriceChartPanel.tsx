...
export const LivePriceChartPanel = React.memo(function LivePriceChartPanel() {
  ...
  const [priceStatus, setPriceStatus] = useState<"loading" | "live" | "error">(
    "loading"
  );
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const seed =
        existing.length > 0 ? existing : [{ time: now, price: realPrice }];
  ...
  return (
    <div className="surface p-6 sm:p-8 space-y-4">
      <h3 className="text-xs font-bold uppercase tracking-wider font-display text-ink flex items-center space-x-2">
        <TrendingUp className="w-4 h-4 text-inkblue" />
      ...
      <div className="flex items-center gap-3 mb-3 flex-wrap">
        <div className="flex items-center gap-2">
          {priceStatus === "live" && (
            <span className="relative flex h-2 w-2">
              <span className="animate-ping absolute inline-flex h-full w-full rounded-[2px] bg-cyan opacity-75" />
              <span className="relative inline-flex rounded-[2px] h-2 w-2 bg-cyan" />
            </span>
          )}
          {priceStatus === "loading" && (
          ...
          {priceStatus === "error" && (
            <span className="h-2 w-2 rounded-[2px] bg-magenta" />
          )}
          <span className="text-xs font-mono text-ash">
            {priceStatus === "loading" && "Fetching price..."}
            {priceStatus === "live" &&
              `LIVE · SOL/USD · every ${POLL_MS / 1000}s`}
        ...
        {currentPrice > 0 && (
          <div className="ml-auto flex items-center gap-2">
            <span
              className={`text-[21px] font-bold font-mono ${
                currentPrice >= prevPrice ? "text-grass" : "text-magenta"
              }`}
            >
            ...
            >
              ...
              {Math.abs(currentPrice - prevPrice).toFixed(4)}
            </span>
          </div>
        )}
      </div>
      ...
      {priceStatus === "error" ? (
        <div className="flex items-center justify-center h-64 border border-hairline rounded-[8px] bg-cream font-mono text-[13px] text-ash">
