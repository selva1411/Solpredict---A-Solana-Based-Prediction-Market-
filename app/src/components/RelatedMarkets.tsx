  ...
  const { data: markets = [], isLoading } = useQuery({
    ...
    queryFn: async (): Promise<UiMarket[]> => {
      const res = await fetch(
        `/api/markets/cached?category=${encodeURIComponent(
          category
        )}&status=open&limit=8`
      );
  ...
  return (
    <section className="space-y-4">
      <div className="flex items-center justify-between">
        <h3 className="text-[13px] font-bold uppercase tracking-wider font-display text-ink flex items-center gap-2">
          <Link2 className="w-4 h-4 text-inkblue" />
