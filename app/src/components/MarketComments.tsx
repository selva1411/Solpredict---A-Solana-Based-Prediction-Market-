...
export function MarketComments({ marketPubkey }: { marketPubkey: string }) {
  ...
  const fetchComments = async () => {
    try {
      if (!marketPubkey || marketPubkey === "11111111111111111111111111111111")
        return;
    ...
    try {
      ...
      const res = await userFetch(`/api/markets/${marketPubkey}/comments`, {
        ...
        body: JSON.stringify({
          ...
          authorUsername: `${wallet.publicKey
            .toBase58()
  ...
  const handleUpvote = async (comment: Comment) => {
    ...
    setComments((prevComments) =>
      prevComments.map((c) =>
        c.id === comment.id ? { ...c, upvotes: prev + 1 } : c
      )
    );
    try {
      const auth = await signUserProof(wallet, wallet.signMessage).catch(() => null);
      ...
      if (!data?.ok) {
        setComments((prevComments) =>
          prevComments.map((c) =>
            c.id === comment.id ? { ...c, upvotes: prev } : c
          )
        );
      }
    } catch {
  ...
  const CommentItem = ({
    ...
    return (
      <div className={depth > 0 ? "ml-5 border-l border-hairline pl-3" : ""}>
        <div className="p-3 rounded-[4px] bg-sheet border border-hairline space-y-1.5">
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-2">
              ...
              <img
                src={
                  ...
                  `https://api.dicebear.com/7.x/identicon/svg?seed=${comment.authorWallet}`
                }
                alt="avatar"
                className="w-5 h-5 rounded-[2px] bg-sheet"
              />
              <span className="font-bold text-inkblue text-[11px]">
                {comment.authorUsername ||
                  `${comment.authorWallet.slice(0, 4)}...`}
              </span>
            ...
            <div className="flex items-center space-x-3">
              <button
                ...
                className="flex items-center gap-1 text-[10px] text-ash hover:text-grass transition-colors cursor-pointer"
              >
              ...
              <button
                onClick={() => {
                  if (!wallet?.publicKey)
                    return toast.error("Please connect your wallet to reply.");
                  setReplyTo(isReplying ? null : (comment.id as number));
                  setReplyText("");
                }}
                className="flex items-center gap-1 text-[10px] text-ash hover:text-inkblue transition-colors cursor-pointer"
              >
          ...
          <p className="text-ink text-[11px] font-sans leading-relaxed">
            {comment.content}
          </p>
  ...
  return (
    <div className="surface p-6 space-y-6">
      ...
      <form onSubmit={handlePostComment} className="space-y-2">
        <div className="flex gap-2">
          <input
            ...
            placeholder={
              wallet?.publicKey
                ? "Share your prediction rationale or technical analysis..."
      ...
      <div className="space-y-3 font-mono text-xs max-h-80 overflow-y-auto pr-1">
        {tree.length === 0 ? (
          <p className="text-center text-[11px] text-ash py-4">
            No comments yet. Be the first trader to post analysis!
          </p>
        ) : (
          tree.map((node, index) => (
