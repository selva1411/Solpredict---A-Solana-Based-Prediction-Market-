 ...
 */
export function DisputeMarketButton({
  marketPubkey,
  status,
  settledAt,
}: DisputeMarketButtonProps) {
  ...
  const withinWindow =
    !settledAt ||
    Date.now() - new Date(settledAt).getTime() < DISPUTE_WINDOW_MS;
  ...
  return (
    <>
      <button
        ...
        className="inline-flex items-center gap-2 px-3.5 py-2 rounded-[4px] border-2 border-ink bg-cream text-ink font-mono text-[11px] uppercase tracking-[.1em] cursor-pointer transition-all hover:bg-yellow hover:text-ink-static snap"
      >
          ...
          <>
            ...
            >
              <div className="flex items-start justify-between mb-4">
                <div>
                  <div className="label-lux !text-magenta mb-1">
                    Contest the outcome
                  </div>
                ...
                <button
                  onClick={() => !submitting && setOpen(false)}
                  className="p-1.5 rounded-[4px] hover:bg-sheet text-ash-dim hover:text-ink transition-colors cursor-pointer"
                >
                  <X className="w-4 h-4" />
                </button>
              </div>
              ...
              {!publicKey ? (
                <p className="text-[13px] text-ash leading-relaxed py-2">
                  Connect your wallet to file a dispute. Disputes are signed by
                  the filer.
                </p>
              ) : (
                <div className="space-y-4">
                      ...
                      {(["YES", "NO"] as const).map((s) => (
                        <button
                          ...
                          className={`py-2.5 rounded border font-display font-bold text-[15px] uppercase tracking-wide cursor-pointer transition-all ${
                            claimedOutcome === s
                              ? s === "YES"
                                ? "border-ink-static bg-grass text-ink-static"
                                : "border-ink bg-cream text-ink"
                              : "border-hairline bg-sheet text-ash hover:text-ink"
                          }`}
                        >
                          {s}
                        </button>
                  ...
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-cream border border-hairline rounded-[4px] px-3 py-2 text-[13px] font-mono text-ink placeholder:text-ash-dim focus:outline-none focus:border-magenta/60 resize-none"
                    />
                    <div className="text-right text-[9px] font-mono text-ash-dim mt-0.5">
                      {reason.length}/1000
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder="https://…"
                      className="w-full bg-cream border border-hairline rounded-[4px] px-3 py-2 text-[13px] font-mono text-ink placeholder:text-ash-dim focus:outline-none focus:border-magenta/60"
                    />
                  </div>

                  <p className="text-[11px] text-ash-dim leading-relaxed">
                    A 0.1 SOL bond is recorded against this dispute and claims
                    are frozen pending admin review. Frivolous disputes may
                    forfeit the bond.
                  </p>
                  ...
                  <button
                    ...
                    onClick={submit}
                    className="w-full h-11 rounded-[4px] bg-ink-fill text-white font-mono text-[11px] uppercase tracking-[.14em] font-bold cursor-pointer transition-all hover:bg-magenta snap disabled:opacity-40 disabled:cursor-not-allowed"
                  >
