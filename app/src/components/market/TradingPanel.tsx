...
export function TradingPanel(p: TradingPanelProps) {
  ...
  if (status !== "Open") {
    return (
      <div className="p-8 text-center">
        <div className="w-14 h-14 rounded-full flex items-center justify-center mx-auto mb-5 bg-cream border border-hairline">
          <AlertCircle className="w-6 h-6 text-magenta" />
        </div>
        <h4 className="font-display font-extrabold text-[18px] text-ink mb-2">
          {status === "Cancelled"
            ? "Board Cancelled"
            : status === "Ended"
            ? "Trading Ended"
        ...
        <p className="text-[13px] text-ash leading-relaxed max-w-[34ch] mx-auto">
          {status === "Cancelled" ? (
            ...
            <>
              ...
              <Link
                href="/dashboard"
                className="text-magenta hover:underline font-bold"
              >
                Dashboard
              </Link>{" "}
              to withdraw payout.
  ...
  return (
    <div className="space-y-0">
      ...
      <div className="grid grid-cols-3 rounded-[8px] p-1 gap-1 bg-sheet border border-hairline">
        {(["buy", "sell", "liquidity"] as const).map((tab) => (
          <button
            key={tab}
            data-testid={`tab-${tab}`}
            onClick={() => setTradeTab(tab)}
            className={`rounded-[4px] py-2 text-[12px] font-medium transition-colors cursor-pointer snap ${
              tradeTab === tab
        ...
        <div className="space-y-4 pt-4">
          ...
          <div className="grid grid-cols-2 gap-3 pt-4">
            ...
            >
              ...
              <span
                className={`font-display font-bold tnum text-[48px] leading-none ${
                  tradeSide === "YES" ? "text-ink" : "text-ash"
                }`}
              >
                {yesProb}¢
              </span>
            ...
            >
              ...
              <span
                className={`font-display font-bold tnum text-[48px] leading-none ${
                  tradeSide === "NO" ? "text-ink" : "text-ash"
                }`}
              >
                {noProb}¢
              </span>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <span className="font-mono text-[10px] text-ash-dim tnum">
                {tradeSide === "YES"
                  ? `${userYesBalance.toFixed(1)} YES held`
                  : `${userNoBalance.toFixed(1)} NO held`}
              </span>
            </div>
            <div className="flex items-center gap-2">
              <button
                onClick={() => setQuantity(Math.max(1, quantity - 1))}
                className="w-10 h-11 rounded-[4px] font-display font-bold text-[16px] text-ink cursor-pointer transition-all snap bg-cream border border-hairline"
                onMouseEnter={(e) =>
                  (e.currentTarget.style.borderColor =
                    "color-mix(in oklab, var(--color-ink) 40%, transparent)")
                }
                onMouseLeave={(e) =>
              ...
              <input
                ...
                className="flex-1 rounded-[4px] px-4 py-3 font-mono text-[18px] font-bold text-ink text-center focus:outline-none transition-all bg-cream border border-hairline"
                onFocus={(e) => {
                  e.currentTarget.style.borderColor =
                    "color-mix(in oklab, var(--color-ink) 70%, transparent)";
                  e.currentTarget.style.boxShadow =
                    "0 0 0 3px color-mix(in oklab, var(--color-ink) 18%, transparent)";
                }}
                onBlur={(e) => {
                  e.currentTarget.style.borderColor = "var(--color-hairline)";
            ...
            <div className="flex gap-1.5 flex-wrap">
              {[10, 25, 50, 100, 250].map((n) => (
                <button
                  ...
                  className="px-2.5 py-1 font-mono text-[10px] text-ash-dim hover:text-ink cursor-pointer transition-colors rounded-[4px] snap bg-cream border border-hairline"
                >
                  {n}
                </button>
              ))}
          ...
          <div className="border border-hairline rounded-[4px] overflow-hidden bg-cream">
            <button
              onClick={() => setShowAdvanced(!showAdvanced)}
              className="w-full flex items-center justify-between px-3 py-2 text-[11px] font-mono text-ash hover:text-ash cursor-pointer transition-colors bg-sheet"
            >
            ...
            {showAdvanced && (
              <div className="px-3 pb-3 pt-2 bg-sheet space-y-3 border-t border-hairline">
                <div className="flex items-center gap-2">
                  <button
                    data-testid="buy-limit-toggle"
                    onClick={() => setIsLimitOrder(!isLimitOrder)}
                    className={`relative w-8 h-4 rounded-[4px] transition-colors cursor-pointer ${
                      isLimitOrder ? "bg-ink-fill" : "bg-hairline"
                    }`}
                  >
                    <span
                      className={`absolute top-0.5 w-3 h-3 bg-cream rounded-[4px] transition-all border border-hairline ${
                  ...
                  <div className="space-y-1">
                    <div className="flex justify-between items-center text-[10px] text-ash uppercase tracking-wider">
                      ...
                      <button
                        ...
                        className="text-magenta hover:underline font-mono"
                      >
                    ...
                    <input
                      ...
                      className="w-full bg-cream border border-hairline rounded-[4px] px-3 py-1.5 text-[13px] font-mono text-ink focus:outline-none focus:border-ink"
                    />
                  </div>
                )}
              </div>
          ...
          <div className="data-cell space-y-2">
            <div className="flex items-center justify-between text-[12px]">
              ...
              <span className="font-mono font-bold text-ink tnum num">
            ...
            <div className="flex items-center justify-between text-[12px]">
              ...
              <span
                className={`font-mono font-bold tnum ${
                  tradeSide === "YES" ? "text-yes" : "text-no"
                }`}
              >
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            className={`w-full h-14 font-semibold text-[13px] tracking-wide cursor-pointer transition-colors rounded-[4px] snap disabled:opacity-40 disabled:cursor-not-allowed ${
              tradeSide === "YES"
                ? "bg-yes-fill text-ink"
                : "bg-no-fill text-white"
            }`}
          >
            {submitting
              ? "Processing..."
          ...
          {/* Tx status */}
          {txState === "signing" && (
            <p className="text-center text-xs font-mono text-magenta animate-pulse">
              Approve in wallet...
            </p>
          )}
          {txState === "confirming" && (
            <p className="text-center text-xs font-mono text-inkblue animate-pulse">
              Confirming on-chain...
            </p>
          )}
          {txState === "success" && txSig && (
            <p className="text-center text-xs font-mono text-grass">
              ✓ Done —{" "}
              <a
          ...
          {txState === "error" && (
            <p className="text-center text-xs font-mono text-no">
              Transaction failed
            </p>
          )}
            ...
            <div className="pt-2 border-t border-hairline space-y-2">
              ...
              {userOrders.map((ordAcc, idx) => {
                ...
                return (
                  <div
                    key={idx}
                    className="flex items-center justify-between p-2 rounded-[4px] bg-cream border border-hairline text-[10px] font-mono"
                  >
                    <div>
                      <span
                        className={
                    ...
                    <button
                      ...
                      className="text-no hover:text-no/80 cursor-pointer underline"
                    >
                      Cancel
                    </button>
                  </div>
      ...
      tradeTab === "sell" ? (
        <div className="space-y-4 pt-4">
          ...
          <div className="grid grid-cols-2 gap-3 pt-4">
            <div
              className="p-4 rounded-[8px] text-center surface"
              style={{
                borderTop: "4px solid var(--color-yes)",
              }}
            >
              <div className="font-mono text-[9px] uppercase tracking-wider text-yes mb-1">
                YES Shares
              </div>
              <div className="font-display font-extrabold text-[28px] text-ink leading-none num">
            ...
            <div
              className="p-4 rounded-[8px] text-center surface"
              style={{
                borderTop: "4px solid var(--color-no)",
              }}
            >
              <div className="font-mono text-[9px] uppercase tracking-wider text-no mb-1">
                NO Shares
              </div>
              <div className="font-display font-extrabold text-[28px] text-ink leading-none num">
          ...
          <div className="grid grid-cols-2 gap-2">
            {(["YES", "NO"] as const).map((s) => (
              <button
                ...
                className={`py-2.5 rounded-[4px] border-2 text-[13px] font-bold uppercase tracking-wide cursor-pointer transition-all snap ${
                  sellSide === s
                    ? s === "YES"
                      ? "border-yes-fill bg-yes-fill text-ink"
                      : "border-no-fill bg-no-fill text-white"
                    : "border-hairline bg-cream text-ash hover:border-ink hover:text-ink"
                }`}
              >
                {s}
              </button>
          ...
          <div className="space-y-2">
            <div className="flex items-center justify-between">
              ...
              <button
                ...
                className="text-[10px] text-magenta hover:underline cursor-pointer font-mono font-bold"
              >
                MAX (
                {Math.floor(
                  sellSide === "YES" ? userYesBalance : userNoBalance
            ...
            <div className="flex items-center gap-2">
              <button
                ...
                className="w-10 h-10 rounded-[4px] bg-cream border border-hairline text-ink font-mono font-bold cursor-pointer snap"
              >
                −
              </button>
              <input
                ...
                className="flex-1 bg-cream border border-hairline rounded-[4px] px-3 py-2 text-center text-[13px] font-mono text-ink focus:outline-none focus:border-ink"
              />
              <button
          ...
          {/* Summary */}
          <div className="bg-cream rounded-[4px] border border-hairline p-3 space-y-2 text-[11px] font-mono">
            ...
            <div className="flex justify-between border-t border-hairline pt-2">
              ...
              <span className="text-grass font-bold num">
                {isLimitOrder
          ...
          {!isLimitOrder && sellQuantity > 0 && sellUnavailable && (
            <div className="bg-no/10 border border-no/40 rounded-[4px] p-2.5 text-[10px] font-mono text-no leading-snug">
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.97 }}
            className={`w-full h-13 font-semibold text-[13px] tracking-wide cursor-pointer transition-colors rounded-[4px] snap disabled:opacity-40 disabled:cursor-not-allowed ${
              sellSide === "YES"
                ? "bg-yes-fill text-ink"
                : "bg-no-fill text-white"
            }`}
        ...
        /* ════════════════════════════════════════════════════════════════
         ...
         ════════════════════════════════════════════════════════════════ */
        <div className="space-y-4 pt-4">
          <div className="space-y-1 bg-cyan/5 border border-cyan/30 p-3 rounded-[4px] font-mono text-[10px] text-ash leading-normal">
          ...
          {(userLp || marketLpStats) && (
            <div className="bg-cream rounded-[4px] border border-hairline p-3 space-y-2 text-[10px] font-mono text-ash">
              {userLp && (
                <>
                  <div className="text-[9px] uppercase tracking-wider text-inkblue font-bold">
                    Your LP in this market
                  </div>
                  <div className="flex justify-between">
                    <span>Deposited:</span>
                    <span className="text-ink font-bold num">
                      {Number(userLp.deposited ?? 0).toFixed(2)} SOL
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>LP Tokens:</span>
                    <span className="text-magenta font-bold num">
                      {Number(userLp.lpShares ?? 0).toLocaleString()} LP
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span>Fees Earned:</span>
                    <span className="text-grass num">
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {(["balanced", "yes", "no"] as const).map((opt) => (
                <button
                  ...
                  className={`py-2 px-1 rounded-[4px] text-[10px] font-bold uppercase tracking-wide cursor-pointer transition-all border snap ${
                    lpOption === opt
                      ? "border-ink-fill bg-ink-fill text-white"
                      : "border-hairline bg-cream text-ash hover:border-ink hover:text-ink"
                  }`}
                >
                  {opt === "balanced"
          ...
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full bg-cream border border-hairline rounded-[4px] px-3 py-2 text-ink focus:outline-none focus:border-ink font-mono text-[13px]"
            />
          </div>
          ...
          <div className="bg-cream rounded-[4px] border border-hairline p-3 space-y-2 text-[10px] font-mono text-ash">
            <div className="flex justify-between">
              ...
              <span className="text-ink num">{yesPool.toFixed(2)} SOL</span>
            </div>
            <div className="flex justify-between">
              <span>Current NO Pool:</span>
              <span className="text-ink num">{noPool.toFixed(2)} SOL</span>
            </div>
            <div className="flex justify-between border-t border-hairline pt-2 text-inkblue">
              <span>New YES Pool:</span>
              <span className="num">{lpNewYesPoolSol.toFixed(2)} SOL</span>
            </div>
            <div className="flex justify-between text-inkblue">
              <span>New NO Pool:</span>
              <span className="num">{lpNewNoPoolSol.toFixed(2)} SOL</span>
            </div>
            <div className="flex justify-between border-t border-hairline pt-2">
              <span>LP Tokens Minted:</span>
              <span className="text-magenta num">
          ...
          <button
            ...
            className="w-full h-12 rounded-[4px] text-white font-semibold text-[12px] tracking-wide cursor-pointer transition-colors flex items-center justify-center gap-2 disabled:opacity-40 disabled:cursor-not-allowed bg-ink-fill hover:bg-ink-fill-soft snap"
          >
            {submitting
              ? "Processing..."
