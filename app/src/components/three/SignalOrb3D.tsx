...
function ProbabilityCore({ yesProbability }: CoreProps) {
  ...
  const lean = useMemo(
    () => Math.max(0, Math.min(1, yesProbability)),
    [yesProbability]
  );
  const color = useMemo(() => {
    ...
    const cyan = new THREE.Color(resolveToken("--color-cyan"));
    const yes = new THREE.Color(resolveToken("--color-yes"));
    const no = new THREE.Color(resolveToken("--color-no"));
  ...
  useFrame((state, delta) => {
    ...
    if (innerRef.current) {
      const s =
        0.92 +
        Math.sin(t * (1.2 + conviction * 2.4)) * (0.045 + conviction * 0.05);
...
export default function SignalOrb3D({
    ...
    return (
      ...
      >
        <div className="flex items-center justify-center w-full h-full">
          <div className="h-32 w-32 rounded-full border border-hairline bg-[radial-gradient(circle_at_35%_30%,color-mix(in_oklab,var(--color-cyan)_25%25,transparent),transparent_65%25)] animate-pulse-slow" />
        </div>
      </div>
    );
  }
  ...
  return (
    ...
    >
        ...
        onCreated={({ gl }) => {
          ...
          gl.domElement.addEventListener(
            ...
            (e) => {
              ...
              setContextLost(true);
              console.warn(
                "[SignalOrb3D] WebGL context lost (onCreated), prevented default"
              );
            },
            false
          );
      ...
      {contextLost && (
        <div className="absolute inset-0 flex items-center justify-center bg-sheet/80 z-10">
          <div className="text-center p-4">
            <div className="text-xs font-mono text-ash mb-2">
              WebGL context lost — click to retry
            </div>
            <button
              onClick={() => window.location.reload()}
              className="text-sm font-mono text-inkblue hover:text-ink underline"
            >
              Reload Page
            </button>
          </div>
