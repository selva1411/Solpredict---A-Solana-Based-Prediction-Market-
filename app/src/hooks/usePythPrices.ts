...
export function usePythPrices(
  ...
  const validHermesFeeds = rawDeduped.filter(
    (id) => isValidPythHex(id) && !failedFeedIds.has(id)
  );
  ...
  const { data } = useQuery({
    ...
    queryFn: async (): Promise<Record<string, PythPriceData>> => {
      ...
      const url = `https://hermes.pyth.network/v2/updates/price/latest?${validHermesFeeds
        .map((id) => `ids%5B%5D=${id}`)
        .join("&")}`;
