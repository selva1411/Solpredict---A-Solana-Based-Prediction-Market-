...
export function useProgram() {
  ...
  const program = useMemo(() => {
    ...
    const parseCategory = (
      categoryObj: AnchorEnumObj | number | null | undefined
    ): number => {
    ...
    const parseComparison = (
      comparisonObj: AnchorEnumObj | number | null | undefined
    ): number => {
    ...
    const originalMarket = (rawAccount as Record<string, unknown>)
      .market as object;
        ...
        if (prop === "account") {
          ...
          return new Proxy(targetAccount, {
            get(accountTarget: object, accountProp: string | symbol) {
              ...
              return typeof val === "function"
                ? (val as (...args: unknown[]) => unknown).bind(accountTarget)
                : val;
            },
          });
        }
