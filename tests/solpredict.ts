...
async function ensureTimePassed(targetTs: number) {
  ...
  let currentTime =
    (await connection.getBlockTime(currentSlotVal)) ||
...
describe("SOLPredict Integration Suite", () => {
  ...
  describe("Phase 2: initialize_config + initialize_market", () => {
    ...
    it("Initialize Market successfully", async () => {
      ...
      expect(marketAccount.question).to.equal(
        "Will SOL exceed $250 by Dec 2026?"
      );
  ...
  describe("Phase 3: buy_shares", () => {
    ...
    it("Buyer1 buys 10 YES shares successfully", async () => {
      ...
      const positionPda = getUserPositionPda(
        marketPda,
        buyer1.publicKey,
        program.programId
      );
      ...
      expect(positionAccount.totalSpentLamports.toNumber()).to.equal(
        100_000_000
      );
  ...
  describe("Phase 4: settle_market + Oracle", () => {
    ...
    before(async () => {
      ...
      mockPriceUpdatePda = getMockPriceUpdatePda(
        admin.publicKey,
        program.programId
      );
      ...
      const buyerYesAta2 = getAssociatedTokenAddressSync(
        yesMintPda,
        buyer2.publicKey
      );
      const buyerNoAta2 = getAssociatedTokenAddressSync(
        noMintPda,
        buyer2.publicKey
      );
      const positionPda2 = getUserPositionPda(
        marketPda,
        buyer2.publicKey,
        program.programId
      );
    ...
    it("Fails settlement if oracle price is stale (>60s age)", async () => {
      ...
      const currentMockPda = getMockPriceUpdatePda(
        mockPayer.publicKey,
        program.programId
      );
    ...
    it("settle_market_manual correctly settles a Sports market with outcome=1 (YES wins)", async () => {
      ...
      const buyer1YesAta = getAssociatedTokenAddressSync(
        sportsYesMint,
        buyer1.publicKey
      );
      const buyer1NoAta = getAssociatedTokenAddressSync(
        sportsNoMint,
        buyer1.publicKey
      );
      const buyer2YesAta = getAssociatedTokenAddressSync(
        sportsYesMint,
        buyer2.publicKey
      );
      const buyer2NoAta = getAssociatedTokenAddressSync(
        sportsNoMint,
        buyer2.publicKey
      );
      const positionPda1 = getUserPositionPda(
        sportsMarketPda,
        buyer1.publicKey,
        program.programId
      );
    ...
    it("settle_market (oracle) successfully settles a Tech market with non-zero feed ID", async () => {
      ...
      const mockPayer = Keypair.generate();
      await fundAccount(mockPayer.publicKey, 1);
      const mockPda = getMockPriceUpdatePda(
        mockPayer.publicKey,
        program.programId
      );
      ...
      await program.methods
        .mockCreatePriceUpdate(
          techFeed,
          new anchor.BN(160_00),
          new anchor.BN(1),
          -2,
          new anchor.BN(freshNow)
        )
  ...
  describe("Phase 5: claim_rewards, cancel_market, claim_refund, withdraw_fees", () => {
    ...
    before(async () => {
      try {
        ...
        const buyerYesAta1 = getAssociatedTokenAddressSync(
          yesMintPda,
          buyer1.publicKey
        );
        const buyerNoAta1 = getAssociatedTokenAddressSync(
          noMintPda,
          buyer1.publicKey
        );
        const positionPda1 = getUserPositionPda(
  ...
  describe("Phase 6: Fractional Refund Math bug validation", () => {
    ...
    it("Buys 2 full YES shares, transfers 1.5 YES shares to recipient, cancels market, and refunds recipient exactly 2.5 * share_price SOL", async () => {
      ...
      const buyerPositionPda = getUserPositionPda(
        localMarketPda,
        buyer.publicKey,
        program.programId
      );

      const recipientYesAta = getAssociatedTokenAddressSync(
        localYesMintPda,
        recipient.publicKey
      );
      const recipientNoAta = getAssociatedTokenAddressSync(
        localNoMintPda,
        recipient.publicKey
      );
      const recipientPositionPda = getUserPositionPda(
        localMarketPda,
        recipient.publicKey,
        program.programId
      );
  ...
  describe("Phase 7: sell_shares", () => {
    ...
    before(async () => {
      ...
      const b1YesAta = getAssociatedTokenAddressSync(
        yesMintPda,
        buyer1.publicKey
      );
      const b1NoAta = getAssociatedTokenAddressSync(
        noMintPda,
        buyer1.publicKey
      );
      const pos1 = getUserPositionPda(
      ...
      const b2YesAta = getAssociatedTokenAddressSync(
        yesMintPda,
        buyer2.publicKey
      );
      const b2NoAta = getAssociatedTokenAddressSync(
        noMintPda,
        buyer2.publicKey
      );
      const pos2 = getUserPositionPda(
        marketPda,
        buyer2.publicKey,
        program.programId
      );
  ...
  describe("Phase 9: add_liquidity + remove_liquidity", () => {
    ...
    it("Provider adds liquidity (YES and NO)", async () => {
      ...
      expect(lpAccount.owner.toBase58()).to.equal(
      ...
      try {
        await program.methods
          ...
          .accounts({
            ...
            noMint: noMintPda,
            buyerYesAta: b1YesAta,
            buyerNoAta: getAssociatedTokenAddressSync(
              noMintPda,
              buyer1.publicKey
            ),
            userPosition: pos1,
  ...
  describe("Phase 11: batch_settle", () => {
    it("Batch settle 2 markets manually", async () => {
      ...
      const m1 = await bootstrapMarket(
        configPda,
        "Batch settle 1?",
        "BS1",
        1,
        ...
        0,
        0,
        new anchor.BN(batchTarget),
        new anchor.BN(batchTarget)
      );
      const m2 = await bootstrapMarket(
        configPda,
        "Batch settle 2?",
        "BS2",
        1,
  ...
  describe("Phase 12: propose_market + approve_market", () => {
    ...
    it("Propose a market with 0.1 SOL bond", async () => {
      ...
      const proposalAccount = await program.account.marketProposal.fetch(
        proposalPda
      );
    ...
    it("Non-admin cannot approve a proposal", async () => {
      ...
      const newProposalVault = getProposalVaultPda(
        newProposalId,
        program.programId
      );
    ...
    it("Reject a proposal: closes the account on-chain and slashes the bond", async () => {
      ...
      const rejectProposalPda = getProposalPda(
        rejectProposalId,
        program.programId
      );
      const rejectVaultPda = getProposalVaultPda(
        rejectProposalId,
        program.programId
      );
  ...
  describe("Phase 14: security hardening — slippage guards, mock-oracle settlement, LP ratio, order refund", () => {
      ...
      try {
        await program.methods
          ...
          .accounts({
            ...
            market: m,
            treasury: result.treasuryPda,
            yesMint: result.yesMintPda,
            noMint: result.noMintPda,
            buyerYesAta: b1YesAta,
            buyerNoAta: b1NoAta,
            userPosition: pos1,
            emergencyPause: null,
    ...
    it("remove_liquidity after trades returns proportionally adjusted amounts", async () => {
      ...
      const pYesAta = getAssociatedTokenAddressSync(
        yesMint,
        provider.publicKey
      );
      ...
      await program.methods
        ...
        .accounts({
          ...
          yesMint,
          noMint,
          providerYesAta: pYesAta,
          providerNoAta: pNoAta,
          liquidityPosition: lpPda,
          emergencyPause: null,
        } as any)
        .signers([provider])
      ...
      expect(providerBalAfter - providerBalBefore).to.be.closeTo(
        yesRefund + noRefund,
        1_000_000
      );
      ...
      expect(marketAfter.yesPoolLamports.toNumber()).to.equal(
        yesPoolAfterTrade - yesRefund
      );
      expect(marketAfter.noPoolLamports.toNumber()).to.equal(
        noPoolAfterTrade - noRefund
      );
      expect(marketAfter.yesSupply.toNumber()).to.equal(
        yesSupplyAfterTrade - burnYes
      );
      expect(marketAfter.noSupply.toNumber()).to.equal(
        noSupplyAfterTrade - burnNo
      );
    });
    ...
    it("cancel_order — partially filled buy order refunds the remaining escrowed SOL", async () => {
      ...
      const makerYesAta = getAssociatedTokenAddressSync(
        yesMint,
        maker.publicKey
      );
      const makerNoAta = getAssociatedTokenAddressSync(noMint, maker.publicKey);
      const makerPos = getUserPositionPda(
        m,
        maker.publicKey,
        program.programId
      );
      await program.methods
        ...
        .accounts({
          ...
          yesMint,
          noMint,
          buyerYesAta: makerYesAta,
          buyerNoAta: makerNoAta,
          userPosition: makerPos,
          emergencyPause: null,
        } as any)
        .signers([maker])
        .rpc();
      ...
      const takerYesAta = getAssociatedTokenAddressSync(
        yesMint,
        taker.publicKey
      );
      const takerNoAta = getAssociatedTokenAddressSync(noMint, taker.publicKey);
      const takerPos = getUserPositionPda(
        m,
        taker.publicKey,
        program.programId
      );
      await program.methods
        ...
        .accounts({
          ...
          yesMint,
          noMint,
          buyerYesAta: takerYesAta,
          buyerNoAta: takerNoAta,
          userPosition: takerPos,
          emergencyPause: null,
        } as any)
        .signers([taker])
        .rpc();
      ...
      const orderEscrowAta = getAssociatedTokenAddressSync(
        yesMint,
        orderPda,
        true
      );
      ...
      expect(makerBalAfterCancel - makerBalBeforeCancel).to.be.greaterThan(
        100_000_000
      );
      // Escrow PDA drained.
  ...
  describe("Phase 15: guardian multisig — add_guardian / remove_guardian / set_guardian_threshold", () => {
    ...
    it("add_guardian registers a distinct guardian (admin is the seeded first guardian)", async () => {
      ...
      const pauseAccount = await program.account.emergencyPause.fetch(pausePda);
      const guardians = pauseAccount.guardians.map((g: PublicKey) =>
        g.toBase58()
      );
    ...
    it("remove_guardian succeeds when removal still meets threshold, then fails when it would drop below", async () => {
      ...
      let guardians = pauseAccount.guardians.map((g: PublicKey) =>
        g.toBase58()
      );
  ...
  describe("Phase 16: close_position positive/negative paths", () => {
    ...
    before(async () => {
      ...
      noMintPda = result.noMintPda;
      treasuryPda = result.treasuryPda;
      positionPda = getUserPositionPda(
        marketPda,
        buyer1.publicKey,
        program.programId
      );
