...
import {
  ...
  ERROR_MAP,
} from "../../app/src/lib/error-map";

describe("error-map", () => {
  describe("ERROR_MAP", () => {
    it("contains known error keys", () => {
      expect(ERROR_MAP.Unauthorized).to.equal(
        "Only the program admin can perform this action"
      );
      expect(ERROR_MAP.MarketNotOpen).to.equal(
        "Market is not open for trading"
      );
      expect(ERROR_MAP.AlreadySettled).to.equal(
  ...
  describe("getFriendlyErrorMessage", () => {
    it("returns default for null/undefined input", () => {
      ...
      expect(getFriendlyErrorMessage(undefined)).to.equal(
    ...
    it("maps error codes to friendly messages", () => {
      ...
      expect(getFriendlyErrorMessage(err)).to.equal(
        "Market is not open for trading"
      );
    });
    ...
    it("handles account does not exist", () => {
      ...
      expect(getFriendlyErrorMessage(err)).to.equal(
        "Required account does not exist on-chain."
      );
    });

    it("handles custom program errors", () => {
      expect(
        getFriendlyErrorMessage(new Error("custom program error: 0x1783"))
      ).to.equal(ERROR_MAP.FeeTooHigh);
      expect(
        getFriendlyErrorMessage(new Error("custom program error: 0x177d"))
      ).to.equal(ERROR_MAP.InvalidQuantity);
      expect(
        getFriendlyErrorMessage(new Error("custom program error: 0x1774"))
      ).to.equal(ERROR_MAP.TooEarlyToSettle);
    });
    ...
    it("handles object with message property", () => {
      expect(getFriendlyErrorMessage({ message: "AlreadySettled" })).to.equal(
        "Market has already been settled"
      );
    });
  });
